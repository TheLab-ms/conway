...
func (m *Module) renderSingleMemberView(r *http.Request, ps httprouter.Params) engine.Response {
	...
	err := m.db.QueryRowContext(r.Context(), `
		SELECT m.id, m.access_status, m.name, m.email, m.confirmed, m.created, m.fob_id, m.admin_notes, m.leadership, m.non_billable, m.stripe_subscription_id, m.stripe_subscription_state, m.paypal_subscription_id, m.paypal_last_payment, m.paypal_price, m.discount_type, ba.identifier, rfm.email, m.bill_annually
		FROM members m
		...
		Scan(&mem.ID, &mem.AccessStatus, &mem.Name, &mem.Email, &mem.Confirmed, &mem.Created, &mem.FobID, &mem.AdminNotes, &mem.Leadership, &mem.NonBillable, &mem.StripeSubID, &mem.StripeStatus, &mem.PaypalSubID, &mem.PaypalLastPayment, &mem.PaypalPrice, &mem.DiscountType, &mem.BuildingAccessApprover, &mem.RootFamilyEmail, &mem.BillAnnually)
	if err != nil {
		return engine.Errorf("querying the database: %s", err)
	}
