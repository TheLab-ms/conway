...
describe('default fetch router boundaries', () => {
  it('serves public config and assets with security headers, not private settings', async () => {
    ...
    expect(await response.json()).toMatchObject({ site_name: 'Conway', waiver: null, stripe_enabled: false });
    ...
    expect(config.discounts).toEqual([{ id: 'student', label: 'Student' }, { id: 'family', label: 'Family' }]);
  ...
  it.each(['/api/admin/members', '/api/admin/config', '/api/admin/events', '/api/admin/swipes', '/api/admin/jobs', '/api/admin/export'])('denies non-leaders access to %s', async path => {
    expect((await api(path, 'GET', await login((await member()).id))).status).toBe(403);
  });

  it.each([
    ['/api/profile', 'PATCH'], ['/api/signup', 'POST'], ['/api/logout', 'POST'],
    ['/api/waiver', 'POST'], ['/api/discount', 'DELETE'], ['/api/fobs/bind', 'POST'],
    ['/api/admin/members', 'POST'], ['/api/admin/config', 'PUT'], ['/api/kiosk/claims', 'POST'],
  ...
  it('applies JSON content-type and size limits through the router without changing member data', async () => {
    ...
    const input = new Request(`${env.SITE_URL}/api/profile`, { method: 'PATCH', headers, body: '{"name":"No"}' });
...
describe('member profile and directory', () => {
  it('edits only the logged-in profile and hides admin notes from member/session responses', async () => {
    ...
    const headers = await login(row.id);
    const response = await api('/api/profile', 'PATCH', headers, { name: ' After ', bio: ' Bio ', pronouns: 'they/them', directory_hidden: true, discord_checkin_notify: false });
    expect(response.status).toBe(200);
    expect(await response.json()).toMatchObject({ id: row.id, name: 'After', bio: 'Bio', directory_hidden: 1, discord_checkin_notify: 0 });
  ...
  it.each([{ leadership: 1 }, { non_billable: 1 }, { email: 'changed@example.test' }, { discord_user_id: '999999999999999999' }, { admin_notes: 'overwrite' }, { fob_id: 1 }, { id: 9 }, {}, { directory_hidden: 'false' }])('rejects unauthorized/invalid profile fields %j atomically', async data => {
    const row = await member({ name: 'Original' });
  ...
  it('lists only visible Ready named members, puts self first, and never leaks private columns', async () => {
    ...
    const visible = await member({ name: 'A Visible', non_billable: 1, fob_id: 2, admin_notes: 'Secret', discord_username: 'public-name' });
...
describe('admin configuration and CRUD', () => {
  ...
  it.each([{ unknown: true }, { STRIPE_SECRET_KEY: 'secret' }, { discounts: [{ id: 'same', label: 'One' }, { id: 'same', label: 'Two' }] }, { notification_templates: { arbitrary: 'No' } }, { monthly_price_id: 'not-a-price' }])('rejects invalid settings %j without incrementing version', async fields => {
    const { headers } = await leader();
  ...
  it('creates, searches, updates, exports, and deletes members with actor audit records', async () => {
    ...
    const created = await api('/api/admin/members', 'POST', headers, { email: 'NEW@EXAMPLE.TEST', name: '=Formula, "quoted"', admin_notes: 'Private', non_billable: true });
    expect(created.status).toBe(201);
    const { id } = await created.json<{ id: number }>();
    const read = await (await api(`/api/admin/members/${id}`, 'GET', headers)).json<Member>();
    expect(read).toMatchObject({ email: 'new@example.test', admin_notes: 'Private', non_billable: 1 });
    ...
    expect(events.results).toEqual(['AdminMemberCreated', 'AdminMemberUpdated', 'MemberDeleted'].map(event => ({ actor: actor.id, event })));
  });

  it('maps duplicate Discord/email/fob identities to 409 and leaves existing members unchanged', async () => {
    const { headers } = await leader();
    const original = await member({ email: 'duplicate@example.test', discord_user_id: '987654321098765432', fob_id: 42 });
  ...
  it('protects the last linked leader against delete, demotion, and Discord unlink', async () => {
    ...
    for (const [method, input] of [['DELETE', undefined], ['PATCH', { leadership: false }], ['PATCH', { discord_user_id: null }]] as const) {
  ...
  it('keeps one linked leader when two leaders concurrently demote themselves', async () => {
    ...
    const responses = await Promise.all([
      api(`/api/admin/members/${first.row.id}`, 'PATCH', first.headers, { leadership: false, version: first.row.version }),
      api(`/api/admin/members/${second.id}`, 'PATCH', secondHeaders, { leadership: false, version: second.version }),
    ]);
  ...
  it('only retries dead jobs and atomically clears their retry state', async () => {
    const { row, headers } = await leader();
    const job = await env.DB.prepare("INSERT INTO jobs(kind,payload,status,attempts,lease_until,last_error,completed) VALUES('discord_sync','{}','dead',10,123,'Failure',456) RETURNING id").first<{ id: number }>();
    const response = await api(`/api/admin/jobs/${job!.id}/retry`, 'POST', headers);
    expect(response.status).toBe(200);
    expect(await env.DB.prepare('SELECT status,attempts,lease_until,last_error,completed FROM jobs WHERE id=?').bind(job!.id).first())
      .toEqual({ status: 'pending', attempts: 0, lease_until: 0, last_error: '', completed: null });
...
describe('waiver versions and discounts', () => {
  ...
  it('publishes immutable versions, requires all agreements, deduplicates signing, and rejects stale versions', async () => {
    ...
    const headers = await login(row.id);
    expect((await api('/api/admin/waiver', 'PUT', admin, { content: 'Read this', agreements: ['One', 'Two'] })).status).toBe(201);
    expect((await api('/api/waiver', 'POST', headers, { version: 1, name: 'Signer', agreements: [true, false] })).status).toBe(400);
    for (let i = 0; i < 2; i++) expect((await api('/api/waiver', 'POST', headers, { version: 1, name: 'Signer', agreements: [true, true] })).status).toBe(200);
    ...
    expect((await api('/api/waiver', 'POST', headers, { version: 1, name: 'Signer', agreements: [true, true] })).status).toBe(409);
  ...
  it('rechecks waiver version if publication changes during streaming request body', async () => {
    ...
    const pending = SELF.fetch(`${env.SITE_URL}/api/waiver`, { method: 'POST', headers: { ...headers, 'Content-Type': 'application/json' }, body: stream.readable });
  ...
  it('requires a primary family root for approval and propagates its payment state', async () => {
    ...
    expect(await env.DB.prepare('SELECT discount_status,root_family_member,root_family_member_active FROM members WHERE id=?').bind(row.id).first())
      .toEqual({ discount_status: 'approved', root_family_member: root.id, root_family_member_active: 1 });
  });
});
...
describe('kiosk enrollment and D1 changes() atomicity', () => {
  ...
  it('rejects malformed, expired, and nonexistent tokens without changing an existing fob', async () => {
    const created = await claim();
    await env.DB.prepare('UPDATE enrollment_claims SET expires=?').bind(now() - 1).run();
