...
it('scheduled cleanup removes expired D1 auth/claim/rate-limit records without external effects when automation is disabled', async () => {
  ...
  for (const [key, expires] of [['expired', now() - 1], ['live', now() + 3600]] as const) {
    await env.DB.batch([
  ...
  const external = vi.spyOn(globalThis, 'fetch').mockImplementation(async () => { throw new Error('Unexpected external request'); });
  try {
