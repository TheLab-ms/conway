...
describe('0001_core migration in D1', () => {
  ...
  it('preserves explicit imported IDs/timestamps and derives identifier instead of accepting stale status', async () => {
    const row = await member({ id: 123, created: 1234567890, email: 'legacy@example.test', name: 'Legacy Member' });
    expect(row).toMatchObject({ id: 123, created: 1234567890, identifier: 'Legacy Member', payment_status: null, access_status: 'UnconfirmedEmail' });
...
describe('legacy payment and access status parity', () => {
  it.each([
    ...
    expect((await member({ confirmed, non_billable, stripe_subscription_state, paypal_subscription_id })).payment_status).toBe(expected);
  });
...
describe('families and last linked leader protection', () => {
  it('propagates family payment loss/restoration and resets cached state on unlink', async () => {
    ...
    const child = await member({ confirmed: 1, non_billable: 1, fob_id: 8, root_family_member: root.id });
    ...
    expect(await getMember(child.id)).toMatchObject({ root_family_member_active: 0, access_status: 'FamilyInactive' });
    await env.DB.prepare("UPDATE members SET paypal_subscription_id='restored' WHERE id=?").bind(root.id).run();
    expect(await getMember(child.id)).toMatchObject({ root_family_member_active: 1, access_status: 'Ready' });
    await env.DB.prepare('UPDATE members SET root_family_member=NULL WHERE id=?').bind(child.id).run();
    expect(await getMember(child.id)).toMatchObject({ root_family_member_active: null, access_status: 'Ready' });
  });
  ...
  it.each(['DELETE FROM members WHERE id=?', 'UPDATE members SET leadership=0 WHERE id=?', 'UPDATE members SET discord_user_id=NULL WHERE id=?'])('protects last usable leader: %s', async sql => {
  ...
  it('allows controlled imports with automation disabled and no outbox/audit side effects', async () => {
    ...
    const root = await member({ leadership: 1, discord_user_id: '123456789012345678', confirmed: 1, non_billable: 1 });
    const child = await member({ root_family_member: root.id });
...
describe('transactional trigger outbox', () => {
  it('enqueues creation and Discord changes, but not unrelated profile edits', async () => {
    ...
    expect(JSON.parse(event!)).toMatchObject({ payment_before: null, payment_after: 'ActiveStripe' });
  ...
  it('enqueues a discount request once and clears discount metadata on payment lapse', async () => {
    ...
    expect((await env.DB.prepare("SELECT payload,dedupe_key FROM jobs WHERE kind='discount'").all()).results).toEqual([
      { payload: JSON.stringify({ member_id: row.id, request_id: 'request-1' }), dedupe_key: 'discount:request-1' },
    ]);
    await env.DB.prepare("UPDATE members SET stripe_subscription_state='canceled' WHERE id=?").bind(row.id).run();
    expect(await getMember(row.id)).toMatchObject({ discount_type: null, discount_status: null, discount_request_id: null });
  });

  it('deduplicates swipes, never moves last-seen backwards, and emits badge/denied jobs', async () => {
    const row = await member({ fob_id: 42 });
    const swipe = (uid: string, timestamp: number, allowed: number) => env.DB.prepare('INSERT INTO fob_swipes (uid,timestamp,fob_id,member,allowed) VALUES (?,?,42,?,?) ON CONFLICT(uid) DO NOTHING').bind(uid, timestamp, row.id, allowed).run();
    ...
    expect((await env.DB.prepare("SELECT kind,dedupe_key FROM jobs WHERE kind IN('badge','denied') ORDER BY id").all()).results).toEqual([
      { kind: 'badge', dedupe_key: 'swipe:new' }, { kind: 'denied', dedupe_key: 'swipe:old' },
    ]);
  });
