...
async function signupSession(fields: Record<string, string> = {}) {
  ...
  const pending = { email: 'new@example.test', discord_user_id: '123456789012345678', discord_username: 'new-member', return_to: '/dashboard', ...fields };
  await env.DB.prepare('UPDATE sessions SET signup=? WHERE token_hash=?')
    .bind(JSON.stringify(pending), await hash(headers.Cookie.split('=')[1])).run();
...
describe('session and signup through default fetch', () => {
  it('creates and reuses an anonymous CSRF session without leaking hashes', async () => {
    ...
    expect(data).toEqual({ member: null, signup: null, csrf_token: expect.stringMatching(/^[a-f0-9]{64}$/) });
  ...
  it('enforces the anonymous D1 rate limit per trusted client IP and resets expired buckets', async () => {
    ...
    await env.DB.prepare('INSERT INTO rate_limits(key,count,expires) VALUES(?,60,?)').bind(key, now() + 600).run();
    ...
    expect((await api('/api/session', 'GET', { 'CF-Connecting-IP': ip, 'X-Forwarded-For': '192.0.2.99' })).status).toBe(429);
    expect((await api('/api/session', 'GET', { 'CF-Connecting-IP': '192.0.2.21' })).status).toBe(200);
    await env.DB.prepare('UPDATE rate_limits SET expires=? WHERE key=?').bind(now() - 1, key).run();
  ...
  it('creates a confirmed Discord member and rotates token/CSRF despite member-created triggers', async () => {
    ...
    expect(before.signup).toEqual({ email: pending.email, discord_user_id: pending.discord_user_id });
    const response = await api('/api/signup', 'POST', headers, { name: ' New Member ', heard_about: ' Friend ' });
    expect(response.status).toBe(200);
    ...
    expect(record).toMatchObject({ email: pending.email, confirmed: 1, name: 'New Member', heard_about: 'Friend', discord_user_id: pending.discord_user_id });
    const session = await env.DB.prepare('SELECT * FROM sessions').first<Session>();
    expect(session).toMatchObject({ member: record!.id, signup: null, token_hash: await hash(cookie.split('=')[1]) });
  ...
  it.each(['email', 'discord_user_id'])('does not rotate or authenticate signup on existing %s collision', async field => {
    ...
    const response = await api('/api/signup', 'POST', headers, { name: 'Attacker', heard_about: '' });
  ...
  it('rechecks pending session expiration inside the signup transaction after body streaming', async () => {
    ...
    const response = worker.fetch(new Request(`${env.SITE_URL}/api/signup`, { method: 'POST', headers: { ...headers, 'Content-Type': 'application/json' }, body: stream.readable }), env, ctx);
    await writer.write(new TextEncoder().encode('{"name":"New",'));
    await env.DB.prepare('UPDATE sessions SET expires=?').bind(now() - 1).run();
  ...
  it.each([{ name: '', heard_about: '' }, { name: 'Name' }, { name: 'Name\nInjected', heard_about: '' }, { name: 'x'.repeat(201), heard_about: '' }])('rejects invalid signup input %j without mutation', async data => {
    const { headers } = await signupSession();
  ...
  it('invalidates every existing session when leadership relinks a member to another Discord identity', async () => {
    ...
    const response = await api(`/api/admin/members/${target.id}`, 'PATCH', admin, { discord_user_id: '111111111111111111', version: record.version });
    expect(response.status).toBe(200);
  ...
  it('invalidates sessions on Discord unlink but preserves them for an unchanged identity', async () => {
    ...
    for (const discord_user_id of [target.discord_user_id, null]) {
      const record = await (await api(`/api/admin/members/${target.id}`, 'GET', admin)).json<Member>();
      expect((await api(`/api/admin/members/${target.id}`, 'PATCH', admin, { discord_user_id, version: record.version })).status).toBe(200);
  ...
  it('does not invalidate sessions if a duplicate-identity relink fails', async () => {
    ...
    const headers = await login(target.id);
    const record = await (await api(`/api/admin/members/${target.id}`, 'GET', admin)).json<Member>();
    expect((await api(`/api/admin/members/${target.id}`, 'PATCH', admin, { discord_user_id: administrator.discord_user_id, version: record.version })).status).toBe(409);
...
describe('Discord OAuth with only external HTTP mocked', () => {
  it('renders safe non-cached HTML for GET login errors without reflecting callback input', async () => {
    ...
    const query = new URLSearchParams({ state: payload, code: 'private-authorization-code', error_description: payload, return_to: 'javascript:alert(1)' });
  ...
  it('keeps API and non-GET login errors JSON even when the browser accepts HTML', async () => {
    for (const [path, method, status] of [['/api/member', 'GET', 401], ['/login/discord/callback', 'POST', 404]] as const) {
  ...
  async function begin(returnTo = '/dashboard') {
    ...
    return { state: target.searchParams.get('state')!, cookie: responseCookie(response, 'conway_oauth') };
  }
  ...
  function discord(fields: Record<string, unknown> = {}) {
    return vi.spyOn(globalThis, 'fetch').mockImplementation(async (input, init) => {
      ...
      if (url === 'https://discord.com/api/v10/users/@me') {
        ...
        return Response.json({ id: '123456789012345678', username: 'discord-name', email: 'new@example.test', verified: true, ...fields });
      }
      throw new Error(`Unexpected external request: ${url}`);
    });
  }

  it.each(['https://evil.test', '//evil.test', '/\\evil.test', '/%2f%2fevil.test', '/login/discord', '/api/member'])('sanitizes unsafe return URL %s in persisted OAuth state', async returnTo => {
    const { state } = await begin(returnTo);
    expect(await env.DB.prepare('SELECT return_to FROM oauth_states WHERE state_hash=?').bind(await hash(state)).first('return_to')).toBe('/dashboard');
  });
  ...
  it('requires matching browser cookie and makes successful callback state one-use', async () => {
    ...
    expect(current).toMatchObject({ member: null, signup: { email: 'new@example.test', discord_user_id: '123456789012345678' } });
  });
  ...
  it('rejects expired or duplicate state parameters without contacting Discord', async () => {
    ...
    await env.DB.prepare('UPDATE oauth_states SET expires=?').bind(now() - 1).run();
    expect((await api(path, 'GET', { Cookie: auth.cookie })).status).toBe(400);
    expect(external).not.toHaveBeenCalled();
  });
  ...
  it('authenticates existing members solely by Discord ID, without changing membership contact email', async () => {
    ...
    const response = await api(`/login/discord/callback?state=${auth.state}&code=code`, 'GET', { Cookie: `${auth.cookie}; ${old.Cookie}` });
    expect(response.status).toBe(302);
    expect(response.headers.get('location')).toBe('/directory');
    expect(await env.DB.prepare('SELECT email,discord_email,discord_username FROM members WHERE id=?').bind(row.id).first()).toEqual({ email: 'old@example.test', discord_email: 'different@example.test', discord_username: 'discord-name' });
    expect(await env.DB.prepare('SELECT count(*) n FROM sessions').first('n')).toBe(1);
  ...
  it('refuses to auto-link a matching email with a different Discord identity', async () => {
    ...
    const auth = await begin();
    discord();
    const response = await api(`/login/discord/callback?state=${auth.state}&code=code`, 'GET', { Cookie: auth.cookie });
    expect(response.status).toBe(409);
    expect(response.headers.get('set-cookie')).toBeNull();
  ...
  it('rejects unverified Discord email and consumes failed callback state', async () => {
    ...
    expect((await api(`/login/discord/callback?state=${auth.state}&code=code`, 'GET', { Cookie: auth.cookie })).status).toBe(403);
    expect(await env.DB.prepare('SELECT count(*) n FROM sessions').first('n')).toBe(0);
