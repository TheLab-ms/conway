...
describe('bounded JSON body in the Workers runtime', () => {
  const request = (value?: string, contentType = 'application/json') => new Request(env.SITE_URL, {
  ...
  it('accepts JSON objects with charset and never caches JSON responses', async () => {
    expect(await body(request('{"name":"Member"}', 'application/json; charset=utf-8'))).toEqual({ name: 'Member' });
  ...
  it('counts streamed bytes, cancels oversize input, and releases the reader', async () => {
    ...
    const input = new Request(env.SITE_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: stream });
...
describe('D1-backed session, authorization, and CSRF', () => {
  ...
  it.each(['https://evil.example.test', 'https://members.example.test.evil.test', 'http://members.example.test', 'null', ''])('rejects missing or foreign Origin %s even with valid CSRF token', async origin => {
