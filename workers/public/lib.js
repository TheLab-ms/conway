...
export async function api(path, { method = 'GET', body, signal } = {}) {
  ...
  try {
    response = await fetch(path, { method, headers, credentials: 'same-origin',
      cache: 'no-store', signal, body: body === undefined ? undefined : JSON.stringify(body) });
  } catch (error) {
  ...
  if (!response.ok) {
    const fallback = response.status === 403 ? 'You do not have permission for this action. Try signing in again.' :
      response.status === 401 ? 'Your session has expired. Please sign in again.' : 'The request could not be completed. Please try again.';
    const message = response.status === 409 && path === '/api/admin/config' ? `${data?.error || 'Settings changed.'} Reload the latest version before saving again. Your edits are still here.` : data?.error || fallback;
...
export function field(label, name, value = '', options = {}) {
  ...
  const control = type === 'textarea' ? el('textarea', { id, name, ...attrs }) :
    choices ? el('select', { id, name, ...attrs }, choices.map(choice => {
...
export const date = value => value ? new Date(Number(value) * 1000).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' }) : 'Never';
...
export function form(fields, submitLabel, handler) {
  ...
  const node = el('form', {}, el('div', { class: 'form-fields' }, fields),
    el('div', { class: 'form-footer' }, submit), output);
...
export function heading(title, subtitle, action) {
  return el('div', { class: 'page-heading' }, el('div', {}, el('p', { class: 'eyebrow' }, 'Member workspace'),
    el('h1', { tabindex: '-1' }, title), subtitle && el('p', {}, subtitle)), action && el('div', { class: 'actions' }, action));
}
export function table(headers, rows) {
  return el('div', { class: 'table-wrap', role: 'region', 'aria-label': headers.join(', '), tabindex: '0' },
    el('table', {}, el('thead', {}, el('tr', {}, headers.map(text => el('th', { scope: 'col' }, text)))),
      el('tbody', {}, rows.map(cells => el('tr', {}, cells.map(cell => el('td', {}, cell)))))));
}
export function safeReturn(value, fallback = '/dashboard') {
  if (typeof value !== 'string' || !value || value.length > 2048 || !value.startsWith('/') || value.startsWith('//') || /[\\\x00-\x20\x7f]|%(?:0[0-9a-f]|1[0-9a-f]|5c|7f)/i.test(value)) return fallback;
  const url = new URL(value, location.origin);
  return url.origin === location.origin && !url.pathname.startsWith('//') && !/%2f/i.test(url.pathname) && !url.pathname.startsWith('/login') && !url.pathname.startsWith('/api/') ? url.pathname + url.search : fallback;
}
