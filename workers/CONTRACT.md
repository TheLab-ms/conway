...
- Pending discounts (`discount_status='requested'`) hold new subscription payment: checkout returns 409 with `{error}` instead of creating or reusing a checkout URL. Existing customers are reconciled first and existing subscriptions retain billing portal access, subject to existing revocation/reconciliation safeguards. Approved/admin discounts still require their configured coupon; members without a discount can start full-price checkout.
- Payment lapse preserves pending discount type, status, and request ID, including when checkout reconciliation discovers a canceled subscription. Approved and legacy admin-granted discounts still clear on payment lapse.
