...
- `GET /api/member`; `PATCH /api/profile {name,discord_checkin_notify}`.
...
- `POST /api/billing/checkout {annual:boolean}` returns `{url}`. `POST /webhooks/stripe`; `POST /discord/interactions`.
