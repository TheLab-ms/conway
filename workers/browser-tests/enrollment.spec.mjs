...
async function scan(page, fob = 202) {
    await page.getByLabel('Fob ID', { exact: true }).fill(String(fob));
    await page.getByLabel('Fob ID', { exact: true }).press('Enter');
...
test('anonymous scanner QR, clipboard and separate phone confirmation replace a fob exactly once', async ({ page, browser, login }) => {
    ...
    try {
        ...
        await expect(kiosk.getByRole('heading', { name: 'Fob linked', exact: true })).toBeVisible();
...
test('actual kiosk IP enforcement rejects scans and polling from an untrusted address', async ({ page, context }) => {
    ...
    await page.getByLabel('Fob ID', { exact: true }).fill('202');
    await page.getByRole('button', { name: 'Create enrollment QR' }).click();
...
test('scan rate limit leaves existing claims intact', async ({ page, context }) => {
    ...
    await page.goto('/kiosk');
    await page.getByLabel('Fob ID', { exact: true }).fill('999');
...
test('reset, expiration and SPA navigation stop polling and remove stale QR codes', async ({ page, context }) => {
    ...
    await expect(page.getByLabel('Fob ID', { exact: true })).toBeFocused();
    await expect(page.getByLabel('Fob ID', { exact: true })).toHaveValue('');
    await expect(page.locator('qr-code, .claim-link')).toHaveCount(0);
    const stopped = polls;
    ...
    await expect(page.getByRole('heading', { name: 'Enrollment expired' })).toBeVisible();
    await expect(page.locator('qr-code, .claim-link')).toHaveCount(0);
    await page.getByRole('button', { name: 'Enroll another fob' }).click();
...
test('idle anonymous session expiry retains actionable reload error and reload recovers', async ({ page, context }) => {
    ...
    await page.clock.install();
    await page.goto('/kiosk');
    await expect(page.getByLabel('Fob ID', { exact: true })).toBeVisible();
    ...
    await expect(page.getByRole('button', { name: 'Create enrollment QR' })).toBeEnabled();
    await page.clock.runFor(6000);
    await expect(page.getByRole('alert')).toContainText('Session expired; reload and try again');
    expect(sql('SELECT * FROM enrollment_claims')).toEqual([]);
    await page.reload();
