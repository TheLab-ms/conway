...
const member = {
    ...
    waiver: null,
    payment_status: null,
    access_status: 'MissingWaiver',
    fob_id: null,
    bio: 'Wood, metal, and code.',
    pronouns: '',
    name_override: null,
    heard_about: '',
    admin_notes: '',
...
const config = {
    ...
    discounts: [{ id: 'family', label: 'Family', coupon_id: 'coupon_family' }],
    donations: [{ price_id: 'price_donate', label: '$25 donation' }],
    notification_templates: {
        signup: 'Hello {name}',
        discount: '{name} requested a discount',
        badge: '{name} checked in',
        denied: 'Access denied',
    },
};
let waiver = {
    version: 3,
    content: 'Use tools safely.\n<script>window.unsafe = true</script>',
    agreements: ['I understand the risks.', 'I agree to follow safety rules.'],
};
const jobs = ['dead', 'pending', 'processing', 'completed'].map((status, index) => ({
    id: 11 + index,
    kind: 'discord_sync',
    status,
    attempts: 3,
    available_at: 1700000000,
    last_error: status === 'dead' ? 'Discord unavailable' : '',
}));
const editable = [
    'name',
    'email',
...
const server = createServer(async (req, res) => {
    ...
    if (path.startsWith('/api/')) {
        ...
        if (fail?.path === path && (!fail.method || fail.method === req.method)) {
            res.statusCode = fail.status;
            res.end(JSON.stringify({ error: fail.message }));
            return;
        }
        if (req.method !== 'GET' && (req.headers['x-csrf-token'] !== csrfToken || req.headers.origin !== `http://${req.headers.host}`)) {
            res.statusCode = 403;
            res.end(JSON.stringify({ error: 'Missing CSRF or Origin' }));
            return;
        }
        let data = {};
        if (path === '/api/session') data = { member: anonymous ? null : member, csrf_token: csrfToken, signup };
            ...
            data = {
                ...
                referral_sources: config.referral_sources,
                discounts: config.discounts.map(({ id, label }) => ({ id, label })),
            ...
            if (req.method === 'PUT') {
                if (
                    Object.keys(body).some(
                        (key) =>
                            !Object.keys(config)
                                .filter((key) => key !== 'waiver_version')
                                .includes(key),
                    )
                )
                    return reject(400, 'Unknown setting');
            ...
            member.discount_status = 'approved';
            member.root_family_member = body.root_family_member;
            member.version++;
            data = { approved: true };
            ...
            if (req.method === 'POST') {
                ...
                member.discount_type = body.discount_type;
                member.discount_status = 'requested';
                member.discount_request_id = token;
                data = { requested: true };
            } else {
                member.discount_type = null;
                member.discount_status = null;
                member.discount_request_id = null;
                data = { removed: true };
            }
            member.version++;
            ...
            member.waiver = 1;
            member.version++;
            data = { signed: true, version: waiver.version };
        } else if (path === '/api/admin/waiver') {
            if (Object.keys(body).sort().join(',') !== 'agreements,content') return reject(400, 'Invalid waiver fields');
            waiver = { ...body, version: config.waiver_version + 1 };
            config.waiver_version = waiver.version;
            config.version++;
            data = { version: waiver.version };
            res.statusCode = 201;
        } else if (path === '/api/admin/events')
            data = {
                events: [{ created: 1700000000, member: 7, actor: null, event: 'Updated', details: '<img src=x>' }],
            };
        else if (path === '/api/admin/swipes')
            data = {
                swipes: [
                    {
            ...
            job.status = 'pending';
            data = { retried: true };
        } else if (path === '/api/kiosk/claims') {
            data = {
                token,
                url: `http://${req.headers.host}${bindPath}`,
    ...
    if (file !== resolve(root) && !file.startsWith(root)) {
...
const chrome = spawn(
    ...
    ['--headless=new', '--no-sandbox', '--disable-gpu', '--no-first-run', '--no-default-browser-check', '--remote-debugging-pipe', `--user-data-dir=${profile}`, 'about:blank'],
...
async function wait(expression, timeout = 6000) {
    ...
    while (Date.now() - start < timeout) {
        if (await evaluate(`(${expression}) && !document.querySelector('[aria-busy=true]')`)) return;
...
async function open(path) {
    await cdp('Page.navigate', { url: origin + path }, session);
    await wait(ready);
}
async function route(path) {
    await evaluate(`document.querySelector('conway-app').navigate(${JSON.stringify(path)})`);
    await wait(ready);
}
async function fill(name, value) {
    await evaluate(
        `(() => { const input = document.querySelector('[name=${name}]'); input.value = ${JSON.stringify(value)}; input.dispatchEvent(new Event('input', {bubbles:true})); })()`,
    );
}
async function submit(index = 0) {
    await evaluate(`document.querySelectorAll('main form')[${index}].requestSubmit()`);
}
...
try {
    ...
    await cdp('Log.enable', {}, session);
    await cdp('Network.enable', {}, session);
    await test('dashboard and navigation', async () => {
        await open('/dashboard');
        assert.equal(await evaluate("document.querySelector('h1').textContent"), 'Hello, Alex Maker.');
    ...
    await test('text-only directory and safe search', async () => {
        await route('/directory');
        assert.equal(await evaluate("document.querySelectorAll('member-card').length"), 2);
        ...
        await fill('search', 'Wood');
        assert.equal(await evaluate("document.querySelectorAll('member-card').length"), 1);
        await fill('search', 'not-a-member');
        assert.equal(await evaluate(contains('No members match')), true);
    });
    await test('profile CSRF and boolean payloads', async () => {
        await route('/profile');
        await fill('bio', 'Updated bio');
        await submit();
        await wait(contains('Your profile has been saved.'));
        assert.equal(last('/api/profile').body.bio, 'Updated bio');
        assert.equal(last('/api/profile').body.directory_hidden, false);
        assert.equal(last('/api/profile').headers['x-csrf-token'], 'test-csrf');
    });
    await test('waiver consent and exact version', async () => {
        await route('/waiver');
        await submit();
        assert.equal(last('/api/waiver'), undefined);
        ...
        assert.deepEqual(last('/api/waiver').body, {
            version: 3,
            name: 'Alex Maker',
    ...
    await test('null waiver and first publication editor', async () => {
        ...
        waiver = null;
        await route('/waiver');
        assert.equal(await evaluate(contains('A waiver has not been published yet')), true);
        assert.equal(await evaluate("document.querySelectorAll('main form').length"), 0);
        await route('/admin/waiver');
        assert.equal(await evaluate(contains('Not published')), true);
    ...
    await test('stale waiver requires review of new version', async () => {
        ...
        await evaluate("document.querySelectorAll('input[type=checkbox]').forEach(input => input.checked = true)");
        const saved = waiver;
        waiver = { ...waiver, version: 99 };
        await submit();
        await wait(contains('Waiver changed; reload and read the current version'));
    ...
    await test('discount request/removal and donation', async () => {
        ...
        await evaluate("document.querySelector('[name=confirm]').checked = true");
        await submit(1);
        await wait(contains('Your discount has been removed.'));
        assert.equal(last('/api/discount').method, 'DELETE');
        await route('/donations');
        await submit();
        await wait(contains('invalid link'));
        assert.deepEqual(last('/api/billing/donation').body, { price_id: 'price_donate' });
    });
    await test('member CRUD and approval', async () => {
        await route('/admin/members/new');
        await fill('name', 'New Member');
        await fill('email', 'new@example.test');
        await fill('discord_user_id', '999999999999999999');
        await submit();
        await wait(contains('Member created.'));
        ...
        await fill('admin_notes', 'Reviewed');
        await submit();
        await wait(contains('Member saved.'));
        ...
        Object.assign(member, {
            ...
            discount_request_id: token,
        });
        await route('/admin/members/7');
        await evaluate("document.querySelector('[name=reviewed]').checked = true");
        await submit(1);
        ...
        await submit(1);
        await wait(contains('Discount request approved.'));
        assert.deepEqual(last('/api/admin/members/7/approve').body, {
            request_id: token,
            root_family_member: 8,
        });
        ...
        await fill('confirmation', '7');
        await submit(1);
        await wait(contains('physical deletion is blocked'));
        ...
        stripeHistory = false;
        member.stripe_subscription_state = null;
        await submit(1);
        await wait(contains('Member deleted.'));
    ...
    await test('stale PATCH and DELETE preserve record until latest revision is loaded', async () => {
        ...
        member.non_billable = 0;
        member.version++;
        await submit();
        await wait(contains('Member changed; reload and retry'));
        ...
        await fill('admin_notes', 'Reviewed latest revision');
        await submit();
        await wait(contains('Member saved.'));
    ...
    await test('config conflict preserves edits and version', async () => {
        await route('/admin/config');
        await fill('site_name', 'My unsaved space');
        fail = {
        ...
        await submit();
        await wait(contains('Your edits are still here.'));
        assert.equal(await evaluate("document.querySelector('[name=site_name]').value"), 'My unsaved space');
        assert.equal(last('/api/admin/config').body.version, 4);
        assert.deepEqual(last('/api/admin/config').body.discounts, config.discounts);
        assert.deepEqual(
            Object.keys(last('/api/admin/config').body).sort(),
            Object.keys(config)
                .filter((key) => key !== 'waiver_version')
                .sort(),
        );
        ...
        fail = null;
        await submit();
        await wait(contains('Configuration saved.'));
    ...
    await test('waiver publishing', async () => {
        ...
        await evaluate("document.querySelector('[name=reviewed]').checked = true");
        await submit();
        await wait(contains('New waiver version published.'));
        assert.equal(last('/api/admin/waiver').method, 'PUT');
    ...
    await test('audit, swipes, jobs, export', async () => {
        await route('/admin/events?member=7');
        assert.equal(await evaluate(contains('Updated')), true);
        assert.equal(await evaluate("document.querySelectorAll('img').length"), 0);
        await route('/admin/swipes');
        assert.equal(await evaluate(contains('Denied')), true);
        await route('/admin/jobs');
        assert.equal(await evaluate("document.querySelectorAll('main form').length"), 1, 'Only dead jobs offer retry');
        for (const status of ['dead', 'pending', 'processing', 'completed']) assert.equal(await evaluate(contains(status)), true);
        await submit();
        await wait(contains('queued for retry'));
        assert.equal(await evaluate("document.querySelectorAll('main form').length"), 0);
        await route('/admin/members');
        assert.equal(await evaluate('document.querySelector(\'a[href="/api/admin/export"]\').textContent'), 'Export CSV');
    });
    await test('backend access status filters and empty results', async () => {
        ...
        await route('/admin/members?status=Ready');
        assert.equal(await evaluate(contains('No members match')), true);
        member.access_status = 'MissingWaiver';
    });
    await test('failure and retry', async () => {
        fail = { path: '/api/directory', status: 503, message: 'Temporarily unavailable' };
        await route('/directory');
        assert.equal(await evaluate(contains('Temporarily unavailable')), true);
        fail = null;
        await evaluate("[...document.querySelectorAll('button')].find(button => button.textContent === 'Try again').click()");
        await wait("document.querySelectorAll('member-card').length === 2");
    });
    await test('expired sessions, forbidden mutation, and rate limiting stay actionable', async () => {
        ...
        fail = null;
        await route('/profile');
        await fill('bio', 'Preserve this unsaved bio');
        fail = { path: '/api/profile', status: 403, message: 'Session expired; reload and try again' };
        await submit();
        await wait(contains('Session expired; reload and try again'));
        ...
        await submit();
        await wait(contains('Invalid name'));
        assert.equal(await evaluate("document.querySelector('[name=bio]').value"), 'Preserve this unsaved bio');
        fail = { path: '/api/kiosk/claims', status: 429, message: 'Too many scans; wait a minute' };
        await route('/kiosk');
        await fill('fob_id', '42');
        await submit();
        await wait(contains('Too many scans; wait a minute'));
    ...
    await test('local QR, polling, cleanup', async () => {
        ...
        await fill('fob_id', '42');
        await submit();
        await wait(contains('Use a trusted kiosk.'));
        assert.equal(await evaluate("document.querySelectorAll('canvas').length"), 0);
        fail = null;
        await route('/kiosk');
        await fill('fob_id', '42');
        await submit();
        await wait("!!document.querySelector('qr-code canvas')");
        ...
        claimed = true;
        await wait(contains('has been claimed'));
        assert.equal(await evaluate("document.querySelectorAll('canvas,.claim-link').length"), 0);
    });
    await test('fob confirmation and URL cleanup', async () => {
        await route('/fobs/bind?token=invalid');
        assert.equal(await evaluate(contains('Enrollment link needed')), true);
        await route(bindPath);
        await submit();
        assert.equal(last('/api/fobs/bind'), undefined);
        await evaluate("document.querySelector('[name=confirm]').checked = true");
        await submit();
        await wait(contains('Your access fob is now linked.'));
        assert.deepEqual(last('/api/fobs/bind').body, { token });
    ...
    await test('mobile layout and labels', async () => {
        ...
        for (const path of [
            ...
            await route(path);
            assert.equal(await evaluate('document.documentElement.scrollWidth <= innerWidth'), true, `Overflow on ${path}`);
    ...
    await test('leadership gating', async () => {
        member.leadership = 0;
        const before = requests.filter((request) => request.path === '/api/admin/members').length;
        await open('/admin/members');
        assert.equal(await evaluate(contains('Leadership access required')), true);
    ...
    await test('anonymous login return path', async () => {
        ...
        await open('/');
        assert.equal(await evaluate(contains('Your space to make.')), true);
    });
    ...
    await test('Discord signup returns to enrollment', async () => {
        ...
        await open('/signup?return_to=%2Fprofile');
        await fill('name', 'Alex Maker');
        await fill('heard_about', 'Friend');
        await submit();
        await wait("location.pathname === '/fobs/bind'");
        assert.deepEqual(last('/api/signup').body, { name: 'Alex Maker', heard_about: 'Friend' });
    ...
    await test('history navigation and signout', async () => {
        ...
        await wait("location.pathname === '/dashboard' && !!document.querySelector('h1')");
        await evaluate("[...document.querySelectorAll('button')].find(button => button.textContent === 'Sign out').click()");
        await wait(contains('You have been signed out.'));
        assert.equal(last('/api/logout').headers['x-csrf-token'], 'rotated-csrf');
    });
