...
export async function body<T = Record<string, unknown>>(request: Request): Promise<T> {
  ...
  try {
    for (;;) {
      ...
      if (length > 128 * 1024) { await reader.cancel(); throw new HttpError(413, 'Request too large'); }
      chunks.push(value);
    }
...
export function cookie(request: Request, name: string): string | null {
  const match = (request.headers.get('Cookie') || '').split(';').map(v => v.trim()).find(v => v.startsWith(name + '='));
...
export async function session(request: Request, env: Env): Promise<Session | null> {
  ...
  return env.DB.prepare('SELECT * FROM sessions WHERE token_hash = ? AND expires > ?').bind(await hash(token), now()).first<Session>();
}
...
export function memberView(member: Member): Omit<Member, 'admin_notes'> {
  const { admin_notes: _notes, ...visible } = member; return visible;
}
export async function settings(env: Env): Promise<Settings> {
  const row = await env.DB.prepare('SELECT version, data FROM settings WHERE id = 1').first<{ version: number; data: string }>();
