...
const config = { site_name: 'Lab', monthly_price_id: 'price_month', yearly_price_id: 'price_year',
  donations: [{ price_id: 'price_donation', label: 'Donation' }], discounts: [{ id: 'student', label: 'Student', coupon_id: 'coupon_student' }, { id: 'family', label: 'Family', coupon_id: 'coupon_family' }],
  discord_guild_id: '111', discord_role_id: '222', discord_leadership_channel_id: '333', discord_badge_channel_id: '444',
  signup_notify_enabled: true, badge_notify_enabled: true, access_denied_enabled: true, notification_templates: {} };
const member = (extra = {}) => ({ id: 1, version: 1, root_family_member: null, paypal_subscription_id: null, email: 'member@example.com', name: 'Member', discord_user_id: '555',
  discord_username: 'member', discord_checkin_notify: 1, stripe_customer_id: 'cus_member', stripe_subscription_id: null,
  stripe_subscription_state: null, discount_type: null, discount_status: null, discount_request_id: null,
  payment_status: null, access_status: 'PaymentInactive', ...extra });
const response = (value, status = 200) => new Response(JSON.stringify(value), { status });

function database(handler = () => undefined) {
  const calls = [];
  return { calls, prepare(sql) {
    return { sql: sql.replace(/\s+/g, ' ').trim(), values: [], bind(...values) { this.values = values; return this; },
      async first() { calls.push({ ...this, method: 'first' }); return await handler(this, 'first') ?? null; },
      async all() { calls.push({ ...this, method: 'all' }); return await handler(this, 'all') ?? { results: [] }; },
      async run() { calls.push({ ...this, method: 'run' }); return await handler(this, 'run') ?? { success: true, meta: { changes: 1 } }; } };
...
function harness({ db = database(), fetch: external = async () => { throw new Error('Unexpected fetch'); }, enabled = 'true' } = {}) {
  const storage = new Map();
  const state = { storage: { async get(key) { return structuredClone(storage.get(key)); },
    async put(key, value) { storage.set(key, structuredClone(value)); }, async delete(key) { return storage.delete(key); } } };
  class HttpError extends Error { constructor(status, message) { super(message); this.status = status; } }
  const http = { HttpError, now: () => time, randomToken: () => webcrypto.randomUUID(),
    hash: async value => Buffer.from(await webcrypto.subtle.digest('SHA-256', new TextEncoder().encode(value))).toString('hex'),
    json: (value, status = 200) => response(value, status), body: request => request.json(), settings: async () => config,
    requireMember: async () => member() };
  const context = { exports: {}, require: name => { assert.equal(name, './http'); return http; }, crypto: webcrypto,
    Request, Response, URL, URLSearchParams, TextEncoder, TextDecoder, AbortSignal, Uint8Array, fetch: external, console };
  vm.runInNewContext(compiled.outputText, context, { filename: 'integrations.ts' });
  const api = context.exports;
  const env = { DB: db, AUTOMATION_ENABLED: enabled, SITE_URL: 'https://members.example.com', STRIPE_SECRET_KEY: 'sk_test',
    STRIPE_WEBHOOK_SECRET: 'whsec_test', DISCORD_BOT_TOKEN: 'bot_token', EDGE_URL: 'https://edge.example.com', EDGE_TOKEN: 'edge_token',
    JOBS: { send: async () => {} } };
  const coordinator = new api.MembershipCoordinator(state, env);
  env.COORDINATOR = { idFromName: name => { assert.equal(name, 'stripe'); return name; },
...
async function signedStripe(event, { corrupt = false, timestamp = time } = {}) {
  ...
  return new Request('https://members.example.com/webhooks/stripe', { method: 'POST', body,
...
async function signedDiscord(env, value, { corrupt = false, timestamp = time } = {}) {
  ...
  return new Request('https://members.example.com/discord/interactions', { method: 'POST', body,
    headers: { 'X-Signature-Timestamp': String(timestamp), 'X-Signature-Ed25519': corrupt ? '0'.repeat(128) : signature } });
}
function jobDB(job = {}, who = member(), extra = () => undefined) {
  ...
  return database((statement, method) => {
    ...
    if (statement.sql.startsWith("UPDATE jobs SET status = 'processing'")) {
      ...
      claimed = true;
      return { id: 1, kind: 'signup', payload: '{"member_id":1}', attempts: 1, lease_until: time + 180, ...job };
    }
...
test('Stripe intake atomically deduplicates event and outbox while automation disabled', async () => {
  const batches = [];
  const h = harness({ enabled: 'false', db: database((statement, method) => {
    if (method === 'batch') { batches.push(statement.statements); return []; }
  }) });
  const event = { id: 'evt_test', type: 'checkout.session.completed', data: { object: { id: 'cs_test' } } };
...
const approval = { type: 3, guild_id: '111', channel_id: '333', member: { user: { id: '555' } }, data: { custom_id: 'approve:1:request-123' } };
test('Discord approval requires local leadership and configured guild/channel', async () => {
  for (const change of [{}, { guild_id: 'wrong' }, { channel_id: 'wrong' }]) {
    ...
    assert.equal(h.db.calls.some(call => call.sql.startsWith('UPDATE members')), false);
  }
});
test('approval write rechecks exact pending request and leadership, rejecting stale buttons', async () => {
  const h = harness({ db: database(statement => statement.sql.startsWith('SELECT id FROM members') ? { id: 9 } : undefined) });
...
test('successful approval clears components and audits the local leader actor', async () => {
  const h = harness({ db: database(statement => {
...
test('outbox requeues expired processing jobs and releases failed publication leases', async () => {
  const h = harness({ db: database((_statement, method) => method === 'all' ? { results: [{ id: 7 }] } : undefined) });
  h.env.JOBS.send = async () => { throw new Error('queue unavailable'); };
...
test('Discord 429 retries durably; permanent and exhausted failures enter DLQ', async () => {
  for (const [status, attempts, expected] of [[429, 1, 'pending'], [403, 1, 'dead'], [503, 10, 'dead']]) {
    const h = harness({ db: jobDB({ attempts }), fetch: async () => response({ retry_after: 120 }, status) });
    ...
    assert.equal(h.db.calls.some(call => call.sql.startsWith("UPDATE jobs SET status = 'completed'")), false);
  }
});
...
test('Discord roles follow payment status rather than waiver/fob readiness; no image fetches', async () => {
  const requests = [];
  const h = harness({ db: jobDB({ kind: 'discord_sync' }, member({ payment_status: 'ActiveStripe', access_status: 'MissingWaiver' })),
...
test('messages disable mentions, use stable nonce, and omit image payloads', async () => {
  let sent;
  const h = harness({ db: jobDB(), fetch: async (_url, init) => { sent = JSON.parse(init.body); return response({ id: 'message' }); } });
  await h.internal('/job', { id: 1 });
...
test('badge throttle uses notification state and checks allowed swipe', async () => {
  for (const [allowed, last_sent] of [[0, 0], [1, time - 60]]) {
    const h = harness({ db: jobDB({ kind: 'badge', payload: '{"member_id":1,"swipe_id":"swipe"}' }, member(), statement => {
...
test('coordinator serializes across awaited network calls and durably increases edge versions', async () => {
  ...
  const pushes = [];
  const h = harness({ db: database((_statement, method) => method === 'all' ? { results: [{ fob_id: 42 }] } : undefined),
    fetch: async (url, init) => {
...
test('failed edge pushes consume versions and retry reads fresh authorized set', async () => {
  let fob = 7;
  const pushes = [];
  const h = harness({ db: database((_statement, method) => method === 'all' ? { results: [{ fob_id: fob }] } : undefined),
    fetch: async (url, init) => {
      if (!url.includes('/goal/')) return response({ events: [] });
      pushes.push(JSON.parse(init.body));
...
test('swipes are acknowledged only after durable D1 batch succeeds', async () => {
  for (const fail of [true, false]) {
    const order = [];
    const h = harness({ db: database((_statement, method) => {
      if (method === 'batch') { order.push('commit'); if (fail) throw new Error('D1 unavailable'); }
    }), fetch: async (url, init) => {
      if (url.includes('/goal/')) return response({});
      if (url.includes('/ack')) { order.push('ack'); assert.deepEqual(JSON.parse(init.body).ids, ['edge-1']); return response({}); }
      return response({ events: [{ id: 'edge-1', time: '2026-09-09T12:00:00Z', controller: 'door', fob: 42, allowed: true }] });
    } });
...
test('edge accepts empty 204 responses from versioned push and acknowledgement', async () => {
  const h = harness({ fetch: async url => url.includes('?limit=') ? response({ events: [] }) : new Response(null, { status: 204 }) });
  assert.equal((await h.internal('/edge')).status, 200);
});
test('Stripe reconciliation uses live subscription state instead of stale webhook payload', async () => {
  ...
  const event = { id: 'evt_old', type: 'customer.subscription.deleted', data: { object: { id: 'sub_old', customer: 'cus_member', status: 'canceled' } } };
  const h = harness({ db: jobDB({ kind: 'stripe_event', payload: '{"event_id":"evt_old"}' }, who, statement => {
  ...
  }), fetch: async () => response({ has_more: false, data: [
    { id: 'sub_old', customer: 'cus_member', status: 'canceled', created: 1, metadata: { conway_member_id: '1' } },
    { id: 'sub_new', customer: 'cus_member', status: 'active', created: 2, metadata: { conway_member_id: '1' } },
  ] }) });
  await h.internal('/job', { id: 1 });
...
test('donations deduplicate by session rather than webhook event ID', async () => {
  const event = { id: 'evt_donation', type: 'checkout.session.completed', data: { object: { id: 'cs_donation', customer: 'cus_member' } } };
  const h = harness({ db: jobDB({ kind: 'stripe_event', payload: '{"event_id":"evt_donation"}' }, member(), statement => {
    if (statement.sql.startsWith('SELECT payload, processed')) return { payload: JSON.stringify(event), processed: null };
    if (statement.sql === 'SELECT * FROM members WHERE stripe_customer_id = ?') return member();
  }), fetch: async () => response({ id: 'cs_donation', customer: 'cus_member', mode: 'payment', amount_total: 2500, currency: 'usd', payment_status: 'paid', created: time }) });
  await h.internal('/job', { id: 1 });
  const write = h.db.calls.find(call => call.sql.startsWith('INSERT INTO donations'));
  assert.equal(write.values[0], 'cs_donation');
  assert.match(write.sql, /ON CONFLICT\(id\) DO UPDATE/);
  assert.match(write.sql, /donations.status != 'paid'/);
});
test('manual checkout operates while automation disabled; only approved/admin discounts apply', async () => {
  for (const status of ['requested', 'approved', null]) {
    const forms = [];
    const h = harness({ enabled: 'false', db: database(statement => statement.sql === 'SELECT * FROM members WHERE id = ?' ? member({ discount_type: 'student', discount_status: status }) : undefined),
      fetch: async (url, init) => {
        ...
        return response({ id: 'cs_checkout', url: 'https://checkout.stripe.com/test', status: 'open', expires_at: time + 3600 });
      } });
    assert.equal((await h.internal('/checkout', { member_id: 1, donation: false, annual: true, price_id: '' })).status, 200);
...
test('different concurrent checkout keys share one customer and one open session', async () => {
  ...
  const h = harness({ db: database(statement => {
    ...
    if (url.endsWith('/customers')) { customers++; return response({ id: 'cus_created' }); }
    if (url.includes('/subscriptions?')) return response({ data: [], has_more: false });
    if (init.method === 'POST') sessions++;
    return response({ id: 'cs_checkout', url: 'https://checkout.stripe.com/test', status: 'open', expires_at: time + 3600 });
  } });
  const input = { member_id: 1, donation: false, annual: false, price_id: '' };
...
test('ambiguous customer creation retries with the same persisted Stripe idempotency key', async () => {
  ...
  const h = harness({ db: database(statement => {
    ...
    return response({ id: 'cs_donate', url: 'https://checkout.stripe.com/donation' });
  } });
  const input = { member_id: 1, donation: true, annual: false, price_id: 'price_donation' };
...
test('existing subscriptions use billing portal instead of creating a second subscription', async () => {
  ...
  const h = harness({ db: database(statement => statement.sql === 'SELECT * FROM members WHERE id = ?' ? who : undefined),
    fetch: async (url, init) => {
      calls.push({ url, init });
      if (url.includes('/subscriptions?')) return response({ data: [{ id: 'sub_active', status: 'active', customer: 'cus_member', created: 1 }], has_more: false });
      assert.ok(url.endsWith('/billing_portal/sessions'));
      return response({ url: 'https://billing.stripe.com/portal' });
    } });
  assert.equal((await h.internal('/checkout', { member_id: 1, donation: false, annual: false, price_id: '' })).status, 200);
...
test('family approval SQL requires linked active primary and family checkout fails closed', async () => {
  ...
  for (const root of [null, 1, 2]) {
    const blocked = harness({ db: database(statement => statement.sql === 'SELECT * FROM members WHERE id = ?' ?
      member({ discount_type: 'family', discount_status: 'approved', root_family_member: root }) : undefined),
      fetch: async url => { assert.ok(url.includes('/subscriptions?')); return response({ data: [], has_more: false }); } });
    assert.equal((await blocked.internal('/checkout', { member_id: 1, donation: false, annual: false, price_id: '' })).status, 409);
  }
});

test('definitive Stripe validation rejection clears slot while ambiguous failures retain it', async () => {
  for (const [status, data, clears] of [[400, { error: { type: 'invalid_request_error', code: 'resource_missing' } }, true],
    [500, { error: { type: 'api_error' } }, false], [400, { error: { type: 'idempotency_error' } }, false], [400, {}, false]]) {
    let attempt = 0;
    const keys = [];
    const h = harness({ db: database(statement => statement.sql === 'SELECT * FROM members WHERE id = ?' ? member() : undefined),
    ...
    const input = { member_id: 1, donation: true, annual: false, price_id: 'price_donation' };
    assert.equal((await h.internal('/checkout', input)).status, 502);
    assert.equal(h.storage.has('checkout:1:donation'), !clears);
    assert.equal((await h.internal('/checkout', input)).status, 200);
...
function mutationHarness({ who = member(), leader = true, cas = true, fetch, enabled = 'true', afterRead, root = null } = {}) {
  const batches = [];
  let current = who;
  const h = harness({ enabled, fetch: fetch || (async url => {
    ...
    if (statement.sql === 'SELECT * FROM members WHERE id = ?') { afterRead?.(); return current ? { ...current } : null; }
    ...
    if (method === 'batch') {
      ...
      if (success) {
        if (change.sql.startsWith('DELETE')) current = null;
          ...
          const keys = change.sql.split(' SET ')[1].split(' WHERE ')[0].split(', ').map(part => part.split(' = ')[0]);
          current = { ...current, ...Object.fromEntries(keys.map((key, i) => [key, change.values[i]])), version: current.version + 1 };
        }
      }
      return [{ results: success ? [{ id: 1 }] : [], meta: { changes: success ? 1 : 0 } },
...
test('member mutations require leadership and matching version before any external effects', async () => {
  for (const [leader, version, actor, expected] of [[false, 1, 1, 403], [true, 2, 9, 409]]) {
    const h = mutationHarness({ leader, fetch: async () => { assert.fail('must not fetch'); } });
...
test('self discount removal preserves family link and uses atomic CAS plus conditional audit', async () => {
  const h = mutationHarness({ who: member({ discount_type: 'family', root_family_member: 2 }), root: { id: 2 }, enabled: 'false' });
...
test('post-network CAS conflict fails without claiming success and leaves checkout blocked', async () => {
  ...
  assert.equal((await h.internal('/checkout', { member_id: 1, donation: false, annual: false, price_id: '' })).status, 409);
});

test('revocation expires stored and paginated live sessions before member CAS', async () => {
  const states = new Map([['cs_known', 'open'], ['cs_page1', 'open'], ['cs_page2', 'open']]);
  const calls = [];
  let listed = 0;
  const h = mutationHarness({ who: member({ discount_type: 'student' }), fetch: async (url, init) => {
    ...
    if (parsed.pathname === '/v1/checkout/sessions') {
      ...
      return response({ data: [{ id, customer: 'cus_member', status: states.get(id) }], has_more: listed === 1 });
    }
  ...
  h.storage.set('checkout:1:membership', { key: 'key', created: time, path: '/checkout/sessions', form: {}, result: { id: 'cs_known' } });
...
test('unconfirmed expiration fails closed and preserves pending mutation across retry', async () => {
  const h = mutationHarness({ who: member({ discount_type: 'student' }), fetch: async (url, init) => {
    ...
    if (url.includes('?')) return response({ data: [{ id: 'cs_open', customer: 'cus_member', status: 'open' }], has_more: false });
...
test('deletion blocks PayPal, active/uncertain states, and any Stripe subscription history', async () => {
  for (const who of [member({ paypal_subscription_id: 'paypal' }), member({ stripe_subscription_state: 'active' }), member({ stripe_subscription_state: 'unknown' })]) {
    const h = mutationHarness({ who, fetch: async () => { assert.fail('must block before network'); } });
    assert.equal((await h.api.mutateMember(h.env, 1, null, 9, 1)).status, 409);
    assert.equal(h.batches.length, 0);
  }
  const h = mutationHarness({ fetch: async url => response({ data: url.includes('/subscriptions?') ?
    [{ id: 'sub_old', customer: 'cus_member', status: 'canceled' }] : [], has_more: false }) });
  assert.equal((await h.api.mutateMember(h.env, 1, null, 9, 1)).status, 409);
  assert.equal(h.batches.length, 0);
});
...
test('safe physical deletion audits atomically and cached checkout cannot resurrect deleted member', async () => {
  ...
  h.storage.set(`request:1:membership:${hashed}`, { fingerprint: 'irrelevant', url: 'https://checkout.stripe.com/old' });
  assert.equal((await h.internal('/checkout', { member_id: 1, donation: false, annual: false, price_id: '', request_key: key })).status, 404);
});
...
test('checkout waits for revocation and cannot recreate a discounted session concurrently', async () => {
  ...
  const gate = new Promise(resolve => { release = resolve; });
  let entered = false;
  const h = mutationHarness({ fetch: async url => {
    if (url.includes('/checkout/sessions?') && !entered) { entered = true; await gate; }
  ...
  const checkout = h.internal('/checkout', { member_id: 1, donation: false, annual: false, price_id: '' });
  release();
...
test('revoked cached request key cannot return or recreate its old discounted checkout', async () => {
  ...
  const input = { member_id: 1, donation: false, annual: false, price_id: '', request_key: 'request-old' };
  ...
  h.storage.set(slot, { fingerprint: await digest(JSON.stringify({ donation: false, annual: false, price_id: '' })),
    url: 'https://checkout.stripe.com/old', epoch: 0 });
...
test('completed session racing expiry is reconciled before discount change, not retroactively canceled', async () => {
  ...
  const h = mutationHarness({ who: member({ discount_type: 'student' }), fetch: async (url, init) => {
    if (url.includes('/subscriptions?')) {
      ...
      return response({ data: [{ id: 'sub_completed', customer: 'cus_member', created: 1, status: 'active', metadata: { conway_member_id: '1' } }], has_more: false });
    }
    if (url.includes('/checkout/sessions?')) return response({ data: openList++ ? [] : [{ id: 'cs_raced', customer: 'cus_member', status: 'open' }], has_more: false });
    if (init.method === 'POST') {
      assert.ok(url.endsWith('/expire'));
      order.push('expire');
      return response({ error: { type: 'invalid_request_error' } }, 400);
    }
    return response({ id: 'cs_raced', customer: 'cus_member', status: 'complete', subscription: 'sub_completed' });
  } });
  assert.equal((await h.api.mutateDiscount(h.env, 1, null, 1)).status, 200);
...
test('linked active family primary permits family coupon, inactive primary never does', async () => {
  let coupon;
  const h = harness({ db: database(statement => {
    if (statement.sql === 'SELECT * FROM members WHERE id = ?') return member({ discount_type: 'family', discount_status: 'approved', root_family_member: 2 });
...
test('manual no-billing update needs no network with automation disabled', async () => {
  const h = mutationHarness({ who: member({ stripe_customer_id: null }), enabled: 'false', fetch: async () => assert.fail('No billing mapping, no fetch') });
  assert.equal((await h.api.mutateDiscount(h.env, 1, null, 1)).status, 200);
});
...
test('definitively rejected delete clears marker after safe revocation and permits billing again', async () => {
  const h = mutationHarness({ fetch: async url => {
    ...
    if (url.includes('/subscriptions?')) return response({ data: [{ id: 'sub_old', customer: 'cus_member', status: 'canceled', created: 1 }], has_more: false });
  ...
  assert.equal(h.storage.has('mutation-pending:1'), false);
  assert.equal(h.batches.length, 0);
  for (const donation of [false, true]) {
    assert.equal((await h.internal('/checkout', { member_id: 1, donation, annual: false, price_id: donation ? 'price_donation' : '' })).status, 200);
  }
});

test('delete with unresolved subscription lookup retains pending marker', async () => {
  const h = mutationHarness({ fetch: async url => url.includes('/subscriptions?') ? response({}, 503) : response({ data: [], has_more: false }) });
...
test('inactive family primary does not prevent existing subscriber opening billing portal', async () => {
  const who = member({ discount_type: 'family', discount_status: 'approved', root_family_member: 2,
    stripe_subscription_id: 'sub_active', stripe_subscription_state: 'active' });
  const h = harness({ db: database(statement => statement.sql === 'SELECT * FROM members WHERE id = ?' ? who : undefined),
    fetch: async url => {
      if (url.includes('/subscriptions?')) return response({ data: [{ id: 'sub_active', customer: 'cus_member', status: 'active', created: 1 }], has_more: false });
      assert.ok(url.endsWith('/billing_portal/sessions'));
      return response({ url: 'https://billing.stripe.com/cancel' });
    } });
  const result = await h.internal('/checkout', { member_id: 1, donation: false, annual: false, price_id: '' });
  assert.equal(result.status, 200);
  assert.deepEqual(await result.json(), { url: 'https://billing.stripe.com/cancel' });
  assert.equal(h.db.calls.some(call => call.sql.includes('root_family_member IS NULL')), false);
});
...
test('unrelated and full-form security edits bypass unchanged inactive-family billing during Stripe outage', async () => {
  for (const fullForm of [false, true]) {
    const who = member({ discount_type: 'family', discount_status: 'approved', root_family_member: 2,
      confirmed: 1, non_billable: 0, bill_annually: 0, leadership: 1 });
    let fetched = false;
    const h = mutationHarness({ who, fetch: async () => { fetched = true; return response({}, 503); } });
    const fields = { discord_user_id: null, leadership: 0, ...(fullForm ? {
      discount_type: who.discount_type, discount_status: who.discount_status, discount_request_id: who.discount_request_id,
      root_family_member: who.root_family_member, confirmed: who.confirmed, non_billable: who.non_billable, bill_annually: who.bill_annually,
    } : {}) };
