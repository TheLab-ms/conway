...
async function rateLimit(request: Request, env: Env, kind: 'oauth' | 'anonymous'): Promise<void> {
    ...
    WHERE rate_limits.expires <= ? OR rate_limits.count < ? RETURNING count`,
        ...
        .bind(key, time + 600, time, time, time, kind === 'oauth' ? 20 : 60)
        .first();
    ...
    try {
        ...
        try {
            for (;;) {
                ...
                if (length > 65536) {
                    await reader.cancel();
                    throw new Error('Discord response too large');
                }
                chunks.push(value);
            }
        } finally {
...
export async function authRoute(request: Request, env: Env): Promise<Response | null> {
    ...
    if (url.pathname === '/login/discord' && request.method === 'GET') {
        ...
        await env.DB.prepare('INSERT INTO oauth_states (state_hash, browser_hash, return_to, expires) VALUES (?, ?, ?, ?)')
            .bind(await hash(state), await hash(browser), localReturn(url.searchParams.get('return_to'), env), now() + OAUTH_AGE)
            .run();
        ...
        target.search = new URLSearchParams({
            ...
            redirect_uri: new URL('/login/discord/callback', env.SITE_URL).href,
            state,
        }).toString();
    ...
    if (url.pathname === '/login/discord/callback' && request.method === 'GET') {
        ...
        if (
            ...
            !/^[A-Za-z0-9._~+-]{1,2048}$/.test(token.access_token) ||
        ...
        if (
            ...
            !/^[1-9][0-9]{16,19}$/.test(user.id) ||
            BigInt(user.id) > 18446744073709551615n ||
            typeof user.username !== 'string' ||
            !user.username.trim() ||
            user.username.length > 80 ||
            /[\x00-\x1f\x7f]/.test(user.username) ||
            user.bot === true
        )
    ...
    if (url.pathname === '/api/logout' && request.method === 'POST') {
        ...
        if (browser && OPAQUE.test(browser))
            await env.DB.prepare('DELETE FROM oauth_states WHERE browser_hash = ?')
                .bind(await hash(browser))
                .run();
