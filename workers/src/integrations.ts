...
function parseObject(text: string): ObjectData {
    try {
        ...
        /* Return the same bounded error for malformed JSON. */
    }
    ...
    if (!response.ok) {
        ...
        throw new DeliveryError(
            ...
            service === 'Stripe' &&
                ...
                !String(data.error?.code || '').startsWith('idempotency') &&
                response.headers.get('stripe-should-retry') !== 'true',
        );
    }
    return data;
}
...
export async function mutateMember(env: Env, memberID: number, fields: Record<string, string | number | null> | null, actorID: number, expectedVersion: number): Promise<Response> {
    return coordinated(env, 'stripe', '/member-mutation', {
        member_id: memberID,
        fields,
        actor_id: actorID,
...
export async function mutateDiscount(env: Env, memberID: number, discountType: string | null, expectedVersion: number): Promise<Response> {
    return coordinated(env, 'stripe', '/discount-mutation', {
        member_id: memberID,
...
async function familyEligible(env: Env, member: Member): Promise<boolean> {
    ...
    return Boolean(
        await env.DB.prepare(
            `SELECT id FROM members WHERE id = ? AND root_family_member IS NULL
    AND payment_status IS NOT NULL`,
        )
            .bind(member.root_family_member)
            .first(),
    );
}
...
export async function integrationRoute(request: Request, env: Env): Promise<Response | null> {
    ...
    if (!['/api/billing/checkout', '/webhooks/stripe', '/discord/interactions'].includes(path)) return null;
    ...
    if (path === '/webhooks/stripe') {
        ...
        await env.DB.batch([
            env.DB.prepare('INSERT INTO stripe_events (id, created, payload) VALUES (?, ?, ?) ON CONFLICT(id) DO NOTHING').bind(event.id, now(), text),
            env.DB.prepare(
                `INSERT INTO jobs (kind, payload, available_at, created, dedupe_key)
        SELECT 'stripe_event', ?, ?, ?, ? FROM stripe_events WHERE id = ? AND processed IS NULL
        ON CONFLICT(dedupe_key) DO NOTHING`,
            ).bind(JSON.stringify({ event_id: event.id }), now(), now(), `stripe:${event.id}`, event.id),
        ]);
    ...
    if (path === '/discord/interactions') {
        ...
        if (
            ...
            interaction.channel_id !== env.DISCORD_LEADERSHIP_CHANNEL_ID ||
            !env.DISCORD_LEADERSHIP_CHANNEL_ID
        ) {
        ...
        const approved = await env.DB.prepare(
            `UPDATE members SET discount_status = 'approved'
      ...
      RETURNING id, discount_type`,
            ...
            .bind(Number(match[1]), match[2], leader.id, userID)
            .first<{ id: number; discount_type: string }>();
        ...
        await env.DB.prepare('INSERT INTO member_events (created, member, actor, event, details) VALUES (?, ?, ?, ?, ?)')
            .bind(now(), approved.id, leader.id, 'DiscountApprovedViaDiscord', JSON.stringify({ request_id: match[2], discord_user_id: userID }))
            .run();
        return json({
            type: 7,
            data: {
                content: `Discount approved for member ${approved.id}. Request: ${match[2]}.${approved.discount_type === 'family' ? ' Linked active primary family account verified.' : ''}`,
                embeds: [],
                components: [],
...
export async function runJobs(env: Env): Promise<void> {
    ...
    const due = await env.DB.prepare(
        `UPDATE jobs SET status = 'pending', lease_until = ? WHERE id IN (
    SELECT id FROM jobs WHERE status IN ('pending', 'processing') AND available_at <= ? AND lease_until <= ?
    ORDER BY available_at, id LIMIT 50) RETURNING id`,
    )
        .bind(timestamp + 60, timestamp, timestamp)
        .all<{ id: number }>();
...
export async function processJob(env: Env, id: number): Promise<void> {
    ...
    try {
        payload = parseObject(job.payload);
    } catch {
        /* The leased handler will dead-letter malformed payloads. */
    }
...
async function reconcileStripe(env: Env, member: Member, subscriptions?: ObjectData[]): Promise<void> {
    ...
    const candidates = list.filter(
        (sub: ObjectData) =>
            objectID(sub.customer) === member.stripe_customer_id &&
            (sub.id === member.stripe_subscription_id ||
                sub.metadata?.conway_member_id === String(member.id) ||
    ...
    const sub =
        candidates.find((candidate: ObjectData) => ['active', 'trialing'].includes(candidate.status)) ||
        candidates.find((candidate: ObjectData) => candidate.id === member.stripe_subscription_id) ||
        candidates[0];
    if (!sub) {
    ...
    await env.DB.prepare(
        `UPDATE members SET stripe_subscription_id = ?, stripe_subscription_state = ?,
    ...
    AND (stripe_subscription_id IS NOT ? OR stripe_subscription_state IS NOT ? OR stripe_cancellation_reason IS NOT ? OR stripe_last_payment_error IS NOT ?)`,
        ...
        .bind(sub.id, sub.status, cancellation, paymentError, member.id, member.stripe_customer_id, sub.id, sub.status, cancellation, paymentError)
        .run();
}
...
async function stripeEvent(env: Env, eventID: string): Promise<void> {
    ...
    const relevant =
        ...
        (event.data.object.mode === 'subscription' &&
            ['checkout.session.completed', 'checkout.session.async_payment_succeeded', 'checkout.session.async_payment_failed'].includes(event.type));
    if (relevant) {
        ...
        if (member) {
            if (event.type.startsWith('checkout.session.')) {
                ...
                if (session.mode === 'subscription') await reconcileStripe(env, member);
            ...
            await env.DB.prepare('INSERT INTO member_events (created, event, details) VALUES (?, ?, ?)')
                .bind(now(), 'StripeCustomerUnmapped', JSON.stringify({ event_id: eventID, customer_id: customerID }))
                .run();
        }
    }
...
function denialReason(member: Member): string {
    switch (member.access_status) {
        case 'UnconfirmedEmail':
            return 'Your account is not confirmed. Contact leadership to verify your Discord account.';
        case 'MissingWaiver':
            return 'Sign the current waiver from your membership page.';
        case 'MissingKeyFob':
            return 'Register your key fob at the kiosk, or ask leadership for help.';
        case 'FamilyInactive':
            return 'Your family membership is inactive. Contact the primary account holder.';
        case 'PaymentInactive':
            if (member.stripe_subscription_state === 'past_due')
                return `Your last payment failed${member.stripe_last_payment_error ? `: ${member.stripe_last_payment_error}` : '.'} Update your payment method from your membership page.`;
        ...
        default:
            return 'Access is unavailable. Contact leadership for help.';
    }
}
...
async function discordJob(env: Env, job: Job, payload: ObjectData, state: DurableObjectState): Promise<void> {
    ...
    if (job.kind === 'discord_sync') {
        ...
        if (previous && (previous.user !== member?.discord_user_id || previous.guild !== env.DISCORD_GUILD_ID || previous.role !== env.DISCORD_ROLE_ID)) {
            await discord(
                env,
                `/guilds/${encodeURIComponent(previous.guild)}/members/${encodeURIComponent(previous.user)}/roles/${encodeURIComponent(previous.role)}`,
                'DELETE',
                undefined,
                true,
            );
        ...
        if (!env.DISCORD_GUILD_ID || !env.DISCORD_ROLE_ID) throw new DeliveryError('Discord role sync is not configured');
        const path = `/guilds/${encodeURIComponent(env.DISCORD_GUILD_ID)}/members/${encodeURIComponent(member.discord_user_id)}`;
        await state.storage.put('discord-role', {
            user: member.discord_user_id,
            guild: env.DISCORD_GUILD_ID,
            role: env.DISCORD_ROLE_ID,
        });
        ...
        if (guildMember) {
            ...
            if (guildMember.roles.includes(env.DISCORD_ROLE_ID) !== wanted) {
                await discord(env, `${path}/roles/${encodeURIComponent(env.DISCORD_ROLE_ID)}`, wanted ? 'PUT' : 'DELETE');
            }
        }
        await env.DB.prepare(
            `UPDATE members SET discord_last_synced = ?, discord_username = ?
    ...
    const variables: Record<string, string> = {
        ...
        request_id: member.discount_request_id || '',
        reason: denialReason(member),
...
async function executeJob(env: Env, id: number, state: DurableObjectState): Promise<void> {
    ...
    WHERE id = ? AND available_at <= ? AND (status = 'pending' OR (status = 'processing' AND lease_until <= ?)) RETURNING *`,
        ...
        .bind(timestamp + LEASE_SECONDS, id, timestamp, timestamp)
        .first<Job>();
    if (!job) return;
    try {
        let payload: ObjectData;
        try {
            ...
            throw new DeliveryError('Invalid job JSON', true);
        }
        ...
        await env.DB.prepare(
            `UPDATE jobs SET status = 'completed', completed = ?, lease_until = 0, last_error = ''
      WHERE id = ? AND status = 'processing' AND attempts = ?`,
        )
            .bind(now(), id, job.attempts)
            .run();
    } catch (error) {
        const dead = job.attempts >= MAX_ATTEMPTS || (error instanceof DeliveryError && error.permanent);
        const delay = Math.max(error instanceof DeliveryError ? error.retryAfter : 0, Math.min(21600, 30 * 2 ** Math.min(job.attempts - 1, 10)) + Math.floor(Math.random() * 15));
        await env.DB.prepare(
            `UPDATE jobs SET status = ?, available_at = ?, lease_until = 0, last_error = ?, completed = ?
      WHERE id = ? AND status = 'processing' AND attempts = ?`,
        )
            .bind(dead ? 'dead' : 'pending', now() + delay, (error instanceof Error ? error.message : 'Delivery failed').slice(0, 500), dead ? now() : null, id, job.attempts)
            .run();
    }
}
...
async function ingestSwipes(env: Env): Promise<void> {
    ...
    for (const event of result.events) {
        ...
        if (
            ...
            timestamp < 0 ||
            !/T.*(?:Z|[+-]\d\d:\d\d)$/.test(event.time) ||
            !Number.isSafeInteger(event.fob) ||
            event.fob < 1 ||
            event.fob > 0xffffffff ||
            typeof event.allowed !== 'boolean' ||
            typeof event.controller !== 'string' ||
            event.controller.length > 200
        ) {
        ...
        statements.push(
            env.DB.prepare(
                `INSERT INTO fob_swipes (uid, timestamp, fob_id, member, allowed, controller)
...
export async function scheduled(env: Env): Promise<void> {
    ...
    ORDER BY COALESCE(m.discord_last_synced, 0), m.id LIMIT 50 ON CONFLICT(dedupe_key) DO NOTHING`,
        ...
        .bind(timestamp, timestamp, Math.floor(timestamp / 3600), timestamp - 86400)
        .run();
    const operations: Promise<unknown>[] = [runJobs(env)];
    if (env.STRIPE_SECRET_KEY)
        operations.push(
            coordinated(env, 'stripe', '/reconcile', {}).then(async (response) => {
                if (!response.ok) throw new Error(`Stripe reconciliation HTTP ${response.status}`);
                await response.body?.cancel();
            }),
        );
    if (env.EDGE_URL && env.EDGE_TOKEN)
        operations.push(
            coordinated(env, 'edge', '/edge', {}).then(async (response) => {
    ...
    if (failures.length)
        throw new AggregateError(
            ...
            'Scheduled integration failures',
        );
}
...
export class MembershipCoordinator {
    ...
    private async mutate(slot: string, path: string, form: Record<string, string>): Promise<ObjectData> {
        ...
        if (!operation) {
            operation = {
                key: `conway:${await hash(this.env.SITE_URL)}:${randomToken()}`,
                created: now(),
                path,
                form,
            };
        ...
        try {
            result = await stripe(this.env, path, form, operation.key);
        } catch (error) {
    ...
    private async expireCheckouts(member: Member): Promise<void> {
        ...
        const slot = `checkout:${member.id}:membership`;
        ...
        for (const session of await stripeList(this.env, '/checkout/sessions', query())) {
            if (['payment', 'setup'].includes(session.mode)) continue;
        ...
        for (const [id, session] of sessions) {
            ...
            if (session.status === 'open') {
                try {
                    await this.mutate(`expire:${id}`, `/checkout/sessions/${encodeURIComponent(id)}/expire`, {});
                } catch (error) {
        ...
        if ((await stripeList(this.env, '/checkout/sessions', query())).some((session) => !['payment', 'setup'].includes(session.mode))) {
            throw new HttpError(409, 'Open checkout sessions remain');
        }
    ...
    private async memberMutation(input: ObjectData, selfDiscount: boolean): Promise<Response> {
        ...
        if (selfDiscount) {
            ...
            fields = {
                ...
                discount_status: input.discount_type === null ? null : 'requested',
        ...
        if (fields !== null) {
            ...
            for (const [key, value] of Object.entries(fields)) {
                if (Object.hasOwn(strings, key)) {
                    ...
                    if (value !== null && (!Number.isSafeInteger(value) || Number(value) < 1 || Number(value) > (key === 'fob_id' ? 0xffffffff : Number.MAX_SAFE_INTEGER)))
            ...
            requiresFamilyEligibility =
                next.discount_type === 'family' &&
                next.discount_status !== 'requested' &&
            ...
            if (next.root_family_member !== member.root_family_member && next.root_family_member !== null && next.root_family_member !== undefined) {
                if (
                    next.root_family_member === id ||
                    !(await this.env.DB.prepare('SELECT id FROM members WHERE id = ? AND root_family_member IS NULL').bind(next.root_family_member).first())
                ) {
        ...
        const affectsCheckout =
            fields === null ||
            changedKeys.some((key) =>
                ['discount_type', 'discount_status', 'discount_request_id', 'root_family_member', 'confirmed', 'non_billable', 'bill_annually'].includes(key),
            );
        ...
        if (affectsCheckout || (await this.state.storage.get<boolean>(marker))) {
            ...
            if (member.stripe_customer_id) {
                const subscriptions = await stripeList(
                    this.env,
                    '/subscriptions',
                    new URLSearchParams({
                        customer: member.stripe_customer_id,
        ...
        const mutation =
            fields === null
                ? this.env.DB.prepare(`DELETE FROM members WHERE id = ? AND version = ?${authorization} RETURNING id`).bind(...args)
                : this.env.DB.prepare(
                      `UPDATE members SET ${keys.map((key) => `${key} = ?`).join(', ')} WHERE id = ? AND version = ?${authorization}${familyGuard} RETURNING id`,
        ...
        const result = await this.env.DB.batch<Record<string, unknown>>([
            ...
            this.env.DB.prepare(
                `INSERT INTO member_events (created, member, actor, event, details)
        SELECT ?, ?, (SELECT id FROM members WHERE id = ?), ?, ? WHERE changes() = 1`,
            ).bind(
                now(),
                fields === null ? null : id,
                actor,
                fields === null ? 'MemberDeleted' : selfDiscount ? 'DiscountChanged' : 'AdminMemberUpdated',
                JSON.stringify({
                    member_id: id,
                    actor_id: actor,
    ...
    private async checkout(input: ObjectData): Promise<Response> {
        ...
        if (!member.stripe_customer_id) {
            ...
            const customer = await this.mutate(
                slot,
                '/customers',
                prior?.form || {
                    email: member.email,
        ...
        const coupon = member.discount_type && member.discount_status !== 'requested' ? cfg.discounts.find((item) => item.id === member.discount_type)?.coupon_id : '';
        ...
        const form: Record<string, string> = portal
            ...
            : {
                  mode: 'subscription',
                  customer: member.stripe_customer_id!,
                  success_url: this.env.SITE_URL,
                  cancel_url: this.env.SITE_URL,
                  client_reference_id: String(member.id),
                  'metadata[conway_member_id]': String(member.id),
                  'line_items[0][price]': price!,
                  'line_items[0][quantity]': '1',
                  'subscription_data[metadata][conway_member_id]': String(member.id),
        ...
        if (prior?.result) {
            if (portal) {
                ...
                if (session.status === 'open' && Number(session.expires_at) > now()) {
                    ...
                    if (requestSlot && (await this.state.storage.get(requestSlot)))
                        throw new HttpError(409, 'This checkout request is already complete or expired; start a new request');
                    if (session.status === 'complete' && objectID(session.subscription) !== member.stripe_subscription_id) {
    ...
    private async handle(request: Request): Promise<Response> {
        try {
            ...
            if (path === '/job') {
                ...
                const fobs = await this.env.DB.prepare("SELECT DISTINCT fob_id FROM members WHERE access_status = 'Ready' AND fob_id > 0 ORDER BY fob_id").all<{
                    fob_id: number;
                }>();
                try {
                    await edgeRequest(this.env, '/api/goal/versioned', {
                        version,
                        fobs: fobs.results.map((row) => row.fob_id),
                    });
                ...
                for (const member of members.results) {
                    try {
                        ...
                        await this.env.DB.prepare('INSERT INTO member_events (created, member, event, details) VALUES (?, ?, ?, ?)')
                            .bind(now(), member.id, 'StripeReconcileFailed', error instanceof Error ? error.message : 'Reconciliation failed')
                            .run();
                    }
            ...
            if (error instanceof Error && /constraint failed|Family root|Cannot remove last linked leader/i.test(error.message)) {
                return json(
                    {
                        error: 'Conflicting member identity, fob, family relationship, or last leadership account. Review the record before retrying.',
                    },
                    409,
                );
            }
