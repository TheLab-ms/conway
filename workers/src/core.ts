...
function kiosk(request: Request, env: Env): void {
  ...
  if (!ip || !env.KIOSK_IPS?.split(',').map(s => s.trim()).filter(Boolean).includes(ip)) {
...
async function audit(env: Env, actor: number, member: number | null, event: string, details: unknown): Promise<void> {
  await env.DB.prepare('INSERT INTO member_events(actor,member,event,details) VALUES(?,?,?,?)')
    .bind(actor, member, event, JSON.stringify(details)).run();
}

function editable(input: Input, admin: boolean): Record<string, string | number | null> {
  const fields: Record<string, string | number | null> = {};
  const strings: Record<string, number> = { name: 200, pronouns: 100, bio: 4000,
...
async function validateDiscount(env: Env, fields: Record<string, string | number | null>, previous?: Member): Promise<void> {
  ...
  if (fields.discount_type && fields.discount_type !== previous?.discount_type && !(await settings(env)).discounts.some(d => d.id === fields.discount_type)) throw new HttpError(400, 'Unknown discount type');
...
function validateSettings(input: Input, previous: Settings): Settings {
  ...
  const strings = ['site_name', 'monthly_price_id', 'yearly_price_id', 'discord_guild_id', 'discord_role_id', 'discord_leadership_channel_id', 'discord_badge_channel_id'] as const;
  ...
  for (const key of strings) if (key in input) {
  ...
  if ('discounts' in input) {
    ...
    result.discounts = input.discounts.map((v: Input) => {
      ...
      const id = text(v.id, 'discount ID', 50, false), label = text(v.label, 'discount label', 200, false), coupon_id = text(v.coupon_id ?? '', 'coupon ID', 100);
  ...
  if ('donations' in input) {
    ...
    result.donations = input.donations.map((v: Input) => {
      ...
      const price_id = text(v.price_id, 'price ID', 100, false), label = text(v.label, 'donation label', 200, false);
...
export async function coreRoute(request: Request, env: Env): Promise<Response | null> {
  ...
  if (path === '/api/config' && method === 'GET') {
    ...
    const waiver = await env.DB.prepare('SELECT version,content,agreements FROM waiver_versions WHERE version=?').bind(cfg.waiver_version).first<{ version: number; content: string; agreements: string }>();
    return json({ site_name: cfg.site_name, referral_sources: cfg.referral_sources, discounts: cfg.discounts.map(({ id, label }) => ({ id, label })), donations: cfg.donations,
      waiver: waiver ? { ...waiver, agreements: JSON.parse(waiver.agreements) } : null, stripe_enabled: Boolean(env.STRIPE_SECRET_KEY) });
  }
  if (path.startsWith('/api/kiosk/claims')) {
    kiosk(request, env);
    if (path === '/api/kiosk/claims' && method === 'POST') {
      const input = await body<Input>(request), fob = integer(input.fob_id, 'fob ID', 0xffffffff);
      const count = await env.DB.prepare('SELECT count(*) n FROM enrollment_claims WHERE created > ?').bind(now() - 60).first<{ n: number }>();
      if (count && count.n >= 30) throw new HttpError(429, 'Too many scans; wait a minute');
      const token = randomToken(), expires = now() + 300;
      await env.DB.prepare('INSERT INTO enrollment_claims(token_hash,fob_id,created,expires) VALUES(?,?,?,?)').bind(await hash(token), fob, now(), expires).run();
    ...
    if (claimPath && method === 'GET') {
      const claim = await env.DB.prepare('SELECT claimed_by,expires FROM enrollment_claims WHERE token_hash=?').bind(await hash(claimPath[1])).first<{ claimed_by: number | null; expires: number }>();
  ...
  if (path === '/api/directory' && method === 'GET') {
    const rows = await env.DB.prepare(`SELECT id,name,pronouns,bio,leadership,discord_username FROM members WHERE access_status='Ready' AND directory_hidden=0 AND name!='' ORDER BY id=? DESC,name COLLATE NOCASE,id`).bind(member.id).all();
    return json({ members: rows.results });
  }
  if (path === '/api/profile' && method === 'PATCH') {
    const fields = editable(await body<Input>(request), false), keys = Object.keys(fields);
  ...
  if (path === '/api/waiver' && method === 'POST') {
    const input = await body<Input>(request), version = integer(input.version, 'waiver version'), name = text(input.name, 'signed name', 200, false);
    const current = await env.DB.prepare('SELECT agreements FROM waiver_versions WHERE version=? AND version=(SELECT json_extract(data,\'$.waiver_version\') FROM settings WHERE id=1)').bind(version).first<{ agreements: string }>();
    ...
    if (!Array.isArray(input.agreements) || input.agreements.length !== agreements.length || input.agreements.some(v => v !== true)) throw new HttpError(400, 'All waiver agreements are required');
    const result = await env.DB.prepare(`INSERT INTO waivers(version,name,email) SELECT ?,?,? WHERE ?=(SELECT json_extract(data,'$.waiver_version') FROM settings WHERE id=1) ON CONFLICT(email,version) DO NOTHING RETURNING id`).bind(version, name, member.email, version).first<{ id: number }>();
    if (!result && !(await env.DB.prepare('SELECT id FROM waivers WHERE version=? AND email=?').bind(version, member.email).first())) throw new HttpError(409, 'Waiver changed; reload and read the current version');
    return json({ signed: true, version });
  }
  if (path === '/api/discount' && method === 'POST') {
    const input = await body<Input>(request), type = text(input.discount_type, 'discount type', 50, false);
...
async function adminRoute(request: Request, env: Env): Promise<Response> {
  const leader = await requireLeader(request, env), url = new URL(request.url), path = url.pathname, method = request.method;
  if (path === '/api/admin/config') {
    const cfg = await settings(env);
    if (method === 'GET') return json(cfg);
    if (method === 'PUT') {
      const input = await body<Input>(request), version = integer(input.version, 'settings version');
      const next = validateSettings(input, cfg); delete (next as Partial<Settings>).version;
      const result = await env.DB.prepare('UPDATE settings SET version=version+1,data=? WHERE id=1 AND version=? RETURNING version').bind(JSON.stringify(next), version).first();
  ...
  if (path === '/api/admin/waiver' && method === 'PUT') {
    const input = await body<Input>(request), content = text(input.content, 'waiver content', 60000, false);
    ...
    const result = await env.DB.batch([
      env.DB.prepare('INSERT INTO waiver_versions(version,content,agreements) SELECT COALESCE(MAX(version),0)+1,?,? FROM waiver_versions RETURNING version').bind(content, JSON.stringify(agreements)),
  ...
  if (path === '/api/admin/members' && method === 'GET') {
    const search = (url.searchParams.get('search') || '').slice(0, 200), status = url.searchParams.get('status') || '';
    ...
    const [list, count] = await env.DB.batch([env.DB.prepare(`SELECT * FROM members ${where} ORDER BY id DESC LIMIT 50 OFFSET ?`).bind(...args, offset(url)), env.DB.prepare(`SELECT count(*) total FROM members ${where}`).bind(...args)]);
  ...
  if (path === '/api/admin/members' && method === 'POST') {
    ...
    const row = await env.DB.prepare(`INSERT INTO members(${keys.join(',')}) VALUES(${keys.map(() => '?').join(',')}) RETURNING id`).bind(...keys.map(k => fields[k])).first<{ id: number }>();
  ...
  if (match) {
    ...
    if (match[2] && method === 'POST') {
      const input = await body<Input>(request), requestID = text(input.request_id, 'request ID', 64, false);
      ...
      const result = await env.DB.prepare("UPDATE members SET discount_status='approved',root_family_member=? WHERE id=? AND discount_status='requested' AND discount_request_id=? RETURNING id").bind(root, id, requestID).first();
      if (!result) throw new HttpError(409, 'This discount request is no longer pending');
      await audit(env, leader.id, id, 'DiscountApproved', { request_id: requestID, root_family_member: root });
  ...
  if (path === '/api/admin/events' && method === 'GET') {
    ...
    return json({ events: (await env.DB.prepare('SELECT * FROM member_events WHERE (? IS NULL OR member=?) ORDER BY id DESC LIMIT 50 OFFSET ?').bind(memberID, memberID, offset(url)).all()).results });
  }
  if (path === '/api/admin/swipes' && method === 'GET') return json({ swipes: (await env.DB.prepare('SELECT * FROM fob_swipes ORDER BY timestamp DESC,uid LIMIT 50 OFFSET ?').bind(offset(url)).all()).results });
  if (path === '/api/admin/jobs' && method === 'GET') return json({ jobs: (await env.DB.prepare("SELECT * FROM jobs ORDER BY CASE status WHEN 'dead' THEN 0 WHEN 'processing' THEN 1 WHEN 'pending' THEN 2 ELSE 3 END,id DESC LIMIT 100").all()).results });
  const retry = /^\/api\/admin\/jobs\/(\d+)\/retry$/.exec(path);
  if (retry && method === 'POST') {
    const id = integer(Number(retry[1]), 'job ID');
    const row = await env.DB.prepare("UPDATE jobs SET status='pending',attempts=0,available_at=?,lease_until=0,completed=NULL,last_error='' WHERE id=? AND status='dead' RETURNING id").bind(now(), id).first();
  ...
  if (path === '/api/admin/export' && method === 'GET') {
    ...
    const cell = (value: unknown) => { let s = String(value ?? ''); if (/^[=+@\-\t\r\n]/.test(s)) s = "'" + s; return '"' + s.replaceAll('"', '""') + '"'; };
    return new Response([fields.join(','), ...rows.results.map(row => fields.map(key => cell(row[key])).join(','))].join('\r\n'), { headers: { 'Content-Type': 'text/csv; charset=utf-8', 'Content-Disposition': 'attachment; filename="conway-members.csv"', 'Cache-Control': 'no-store' } });
  }
  throw new HttpError(404, 'Not found');
}
