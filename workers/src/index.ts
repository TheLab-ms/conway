...
async function route(request: Request, env: Env): Promise<Response> {
    ...
    if (path.startsWith('/api/') || path.startsWith('/webhooks/') || path.startsWith('/discord/') || path.startsWith('/login/') || path.startsWith('/oauth2/'))
...
export default {
        ...
        try {
            ...
            if (request.method === 'GET' && new URL(request.url).pathname.startsWith('/login/')) {
                ...
                response = new Response(
                    `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Sign-in unavailable | Conway</title><link rel="stylesheet" href="/styles.css"></head><body><main class="panel"><h1>Sign-in unavailable</h1><p>${escape(message)}</p><a href="/login">Return to sign in</a></main></body></html>`,
                    {
    ...
    async queue(batch: MessageBatch<unknown>, env: Env): Promise<void> {
        for (const message of batch.messages) {
            ...
            if (!value || typeof value.id !== 'number' || !Number.isSafeInteger(value.id) || value.id < 1) {
                message.ack();
                continue;
            }
            try {
                await processJob(env, value.id);
                message.ack();
            } catch {
                message.retry();
            }
