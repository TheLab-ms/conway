...
Features:
  ...
  - MAX3485 RS485 transceiver (GPIO16/17/4) for future accessories
...
JLCPCB Assembly:
  ...
  - THT connectors (J1-J5, J6) require manual soldering
  - Extended parts: EL817 optocouplers ($3 each setup fee)
...
def add_silk_rect(fp, x, y, w, h):
    """Add a silkscreen rectangle outline to a footprint."""
...
def create_sop8(board, ref, val):
    """
    Create SOP-8 / SOIC-8 footprint (MAX3485).
    Standard 1.27mm pitch, 150mil body.
    MAX3485 pins: 1=RO, 2=RE, 3=DE, 4=DI, 5=GND, 6=A, 7=B, 8=VCC
    """
    fp = pcbnew.FOOTPRINT(board)
    fp.SetReference(ref)
    fp.SetValue(val)
...
def create_esp32_wroom_32e(board, ref, val):
    ...
    for i in range(14):
        ...
        add_smd_pad(fp, str(i + 1), -body_w/2 + pad_w/2 - 0.45, y_offset, pad_w, pad_h)

    # Bottom side pads (pins 15-22): y = body_h/2
    for i in range(8):
        x_offset = -body_w/2 + 4.5 + i * pitch  # Start offset from left edge
        add_smd_pad(fp, str(15 + i), x_offset, body_h/2 - pad_h/2 + 0.45, pad_h, pad_w)

    # Right side pads (pins 23-38): x = body_w/2, y from bottom to top
    for i in range(16):
        y_offset = body_h/2 - 1.1 - i * pitch
        add_smd_pad(fp, str(23 + i), body_w/2 - pad_w/2 + 0.45, y_offset, pad_w, pad_h)
...
def create_screw_terminal_2(board, ref, val):
    """Create a 2-pin screw terminal, 5mm pitch, through-hole."""
    fp = pcbnew.FOOTPRINT(board)
    fp.SetReference(ref)
    fp.SetValue(val)
    fp.SetFPIDAsString("Connector_Phoenix_MKDS:PhoenixContact_MKDS_1,5-2_1x02_P5.00mm")
    add_pth_pad(fp, "1", -2.5, 0, 2.4, 1.3)
    add_pth_pad(fp, "2", 2.5, 0, 2.4, 1.3)
    add_fab_rect(fp, 0, 0, 10.4, 8.6)
    add_courtyard_rect(fp, 0, 0, 11.5, 10.0)
    return fp

def create_screw_terminal_4(board, ref, val):
    """Create a 4-pin screw terminal, 5mm pitch, through-hole."""
    fp = pcbnew.FOOTPRINT(board)
    fp.SetReference(ref)
    fp.SetValue(val)
    fp.SetFPIDAsString("Connector_Phoenix_MKDS:PhoenixContact_MKDS_1,5-4_1x04_P5.00mm")
    for i in range(4):
        add_pth_pad(fp, str(i+1), (i - 1.5) * 5.0, 0, 2.4, 1.3)
    add_fab_rect(fp, 0, 0, 20.4, 8.6)
    add_courtyard_rect(fp, 0, 0, 21.5, 10.0)
    return fp
...
def create_nets(board):
    ...
    net_names = [
        ...
        "FTDI_TX", "FTDI_RX",           # FTDI header UART
        "FTDI_DTR", "FTDI_RTS",         # Auto-reset control
        "ESP_EN", "ESP_GPIO0",           # ESP32 boot/reset signals
    ]
    nets = {}
...
def place_components(board, nets):
    ...
    # ---- ESP32 Module ----
    # Place near top-left, with antenna extending toward board edge (top).
    ...
    j1.SetPosition(pt(58.0, 5.5))
    board.Add(j1)
    parts["J1"] = j1

    # D1: SS34 reverse polarity Schottky
    d1 = create_sma_diode(board, "D1", "SS34")
    d1.SetPosition(pt(48.0, 5.5))
    board.Add(d1)
    parts["D1"] = d1

    # U1: AMS1117-3.3 LDO (oriented with tab pointing up/toward board top)
    u1 = create_sot223(board, "U1", "AMS1117-3.3")
    u1.SetPosition(pt(48.0, 14.0))
    ...
    c2 = create_c0805(board, "C2", "100nF")
    c2.SetPosition(pt(42.0, 15.0))
    board.Add(c2)
    parts["C2"] = c2

    # Output filter caps (near U1 output)
    c3 = create_c0805(board, "C3", "22uF")
    c3.SetPosition(pt(55.0, 14.0))
    board.Add(c3)
    parts["C3"] = c3

    c3b = create_c0805(board, "C3B", "22uF")
    c3b.SetPosition(pt(55.0, 17.0))
    board.Add(c3b)
    parts["C3B"] = c3b

    c4 = create_c0805(board, "C4", "100nF")
    c4.SetPosition(pt(55.0, 20.0))
    board.Add(c4)
    parts["C4"] = c4

    # ESP32 decoupling caps (near ESP32 3V3/GND pins)
    c5 = create_c0805(board, "C5", "100nF")
    c5.SetPosition(pt(13.0, 42.0))
    board.Add(c5)
    parts["C5"] = c5

    c6 = create_c0805(board, "C6", "10uF")
    c6.SetPosition(pt(13.0, 45.0))
    board.Add(c6)
    parts["C6"] = c6

    c7 = create_c0805(board, "C7", "10uF")
    c7.SetPosition(pt(13.0, 48.0))
    board.Add(c7)
    parts["C7"] = c7
    ...
    j2.SetPosition(pt(55.0, 30.0))
    board.Add(j2)
    parts["J2"] = j2

    # U2: EL817 optocoupler for D0
    u2 = create_sop4(board, "U2", "EL817")
    u2.SetPosition(pt(43.0, 30.0))
    u2.SetOrientationDegrees(90)
    board.Add(u2)
    parts["U2"] = u2

    # R2: 390R current limiting for U2 LED
    r2 = create_r0805(board, "R2", "390R")
    r2.SetPosition(pt(43.0, 24.0))
    board.Add(r2)
    parts["R2"] = r2

    # R4: 10K pull-up for U2 output
    r4 = create_r0805(board, "R4", "10K")
    r4.SetPosition(pt(43.0, 36.0))
    board.Add(r4)
    parts["R4"] = r4

    # U3: EL817 optocoupler for D1
    u3 = create_sop4(board, "U3", "EL817")
    u3.SetPosition(pt(55.0, 40.0))
    u3.SetOrientationDegrees(90)
    board.Add(u3)
    parts["U3"] = u3

    # R3: 390R current limiting for U3 LED
    r3 = create_r0805(board, "R3", "390R")
    r3.SetPosition(pt(55.0, 34.5))
    board.Add(r3)
    parts["R3"] = r3

    # R5: 10K pull-up for U3 output
    r5 = create_r0805(board, "R5", "10K")
    r5.SetPosition(pt(55.0, 46.0))
    board.Add(r5)
    parts["R5"] = r5
    ...
    j3.SetPosition(pt(8.0, 5.5))
    board.Add(j3)
    parts["J3"] = j3

    # Q1: SS8050 NPN relay driver
    q1 = create_sot23(board, "Q1", "SS8050")
    q1.SetPosition(pt(17.0, 9.0))
    board.Add(q1)
    parts["Q1"] = q1

    # R6: 1K base resistor for Q1
    r6 = create_r0805(board, "R6", "1K")
    r6.SetPosition(pt(23.0, 10.0))
    board.Add(r6)
    parts["R6"] = r6

    # D2: M7 flyback diode
    d2 = create_sma_diode(board, "D2", "M7")
    d2.SetPosition(pt(14.0, 14.0))
    ...
    # J5: 4-pin RS485 screw terminal (A, B, GND, +12V)
    j5 = create_screw_terminal_4(board, "J5", "RS485")
    j5.SetPosition(pt(35.0, 51.0))
    board.Add(j5)
    parts["J5"] = j5
    ...
    # C8: 100nF decoupling for MAX3485
    c8 = create_c0805(board, "C8", "100nF")
    c8.SetPosition(pt(62.0, 50.0))
    board.Add(c8)
    parts["C8"] = c8
    ...
    j6.SetPosition(pt(5.0, 30.0))
    board.Add(j6)
    parts["J6"] = j6

    # Auto-reset circuit: DTR -> C_DTR -> GPIO0, RTS -> C_RTS -> EN
    # R8: 10K pull-up for EN
    r8 = create_r0805(board, "R8", "10K")
    r8.SetPosition(pt(11.0, 20.0))
    board.Add(r8)
    parts["R8"] = r8

    # R9: 10K pull-up for GPIO0
    r9 = create_r0805(board, "R9", "10K")
    r9.SetPosition(pt(11.0, 23.0))
    board.Add(r9)
    parts["R9"] = r9

    # C9: 100nF coupling cap for DTR -> GPIO0 auto-boot
    c9 = create_c0805(board, "C9", "100nF")
    c9.SetPosition(pt(11.0, 26.0))
    board.Add(c9)
    parts["C9"] = c9

    # C10: 100nF coupling cap for RTS -> EN auto-reset
    c10 = create_c0805(board, "C10", "100nF")
    c10.SetPosition(pt(11.0, 29.0))
    board.Add(c10)
    parts["C10"] = c10

    # C11: 100nF bypass on EN for noise filtering
    c11 = create_c0805(board, "C11", "100nF")
    c11.SetPosition(pt(11.0, 17.0))
    board.Add(c11)
    parts["C11"] = c11
...
def assign_nets(parts, nets):
    ...
    # ---- RS485 Section ----
    # U5: MAX3485 (1=RO, 2=RE, 3=DE, 4=DI, 5=GND, 6=A, 7=B, 8=VCC)
...
def route_all(board, parts, nets):
    ...
    rail_x = c1_p[0] - 2.0  # Vertical rail left of caps
    route_wire(board, [d1k, (d1k[0], d1k[1] + 2.0), (rail_x, d1k[1] + 2.0),
                        (rail_x, c1_p[1])], TW_POWER, F, nets["12V"])
    ...
    # 12V to J2.3 (Wiegand reader +12V)
    j2_12v = get_pad_pos(parts["J2"], 3)
    route_wire(board, [d1k, (d1k[0], 3.0), (j2_12v[0], 3.0), j2_12v],
               TW_POWER, F, nets["12V"])

    # 12V to relay section (D2.K) via bottom layer
    d2_k = get_pad_pos(parts["D2"], 1)
    via_12v_relay = (d1k[0] - 10, d1k[1] + 3)
    add_via(board, via_12v_relay, nets["12V"])
    route_wire(board, [d1k, (via_12v_relay[0], d1k[1]), via_12v_relay], TW_POWER, F, nets["12V"])
    route_wire(board, [via_12v_relay, (via_12v_relay[0], d2_k[1]), d2_k],
               TW_POWER, B, nets["12V"])

    # 12V to J5.4 (RS485 +12V) via bottom layer
    j5_12v = get_pad_pos(parts["J5"], 4)
    via_12v_rs485 = (j5_12v[0], j5_12v[1] - 5.0)
    add_via(board, via_12v_rs485, nets["12V"])
    route_wire(board, [via_12v_relay, via_12v_rs485], TW_POWER, B, nets["12V"])
    add_via(board, (j5_12v[0], j5_12v[1] - 2.0), nets["12V"])
    route_wire(board, [via_12v_rs485, (j5_12v[0], j5_12v[1] - 2.0)], TW_POWER, B, nets["12V"])
    route_wire(board, [(j5_12v[0], j5_12v[1] - 2.0), j5_12v], TW_POWER, F, nets["12V"])

    # === 3.3V POWER PATH ===

    # U1.VOUT (pin 2) -> output caps
    ...
    # VOUT -> output caps
    out_rail_x = c3_p[0] + 2.0
    route_wire(board, [u1_tab, (u1_tab[0] + 3, u1_tab[1]),
                        (out_rail_x, u1_tab[1]), (out_rail_x, c3_p[1])],
               TW_POWER, F, nets["3V3"])
    ...
    route_wire(board, [via_3v3_boot, (r8_vcc[0] + 2, via_3v3_boot[1]),
                        (r8_vcc[0] + 2, r8_vcc[1])], TW_PULLUP, B, nets["3V3"])
    route_wire(board, [(r8_vcc[0] + 2, r8_vcc[1]), r8_vcc], TW_PULLUP, F, nets["3V3"])
    route_wire(board, [(r8_vcc[0] + 2, r8_vcc[1]), (r9_vcc[0] + 2, r8_vcc[1]),
                        (r9_vcc[0] + 2, r9_vcc[1])], TW_PULLUP, B, nets["3V3"])
    add_via(board, (r9_vcc[0] + 2, r9_vcc[1]), nets["3V3"])
    route_wire(board, [(r9_vcc[0] + 2, r9_vcc[1]), r9_vcc], TW_PULLUP, F, nets["3V3"])

    # 3.3V to MAX3485 VCC (U5 pin 8) and C8
    u5_vcc = get_pad_pos(parts["U5"], 8)
    c8_p = get_pad_pos(parts["C8"], 1)
    route_wire(board, [u5_vcc, c8_p], TW_POWER, F, nets["3V3"])
    # Connect via from 3V3 rail
    via_3v3_rs = (u5_vcc[0] - 3, u5_vcc[1])
    add_via(board, via_3v3_rs, nets["3V3"])
    route_wire(board, [via_3v3_rs, u5_vcc], TW_POWER, F, nets["3V3"])
    route_wire(board, [via_3v3_pu, (via_3v3_pu[0], 46.0), (via_3v3_rs[0], 46.0), via_3v3_rs],
               TW_POWER, B, nets["3V3"])

    # 3.3V to FTDI J6.3
    j6_vcc = get_pad_pos(parts["J6"], 3)
    via_3v3_ftdi = (j6_vcc[0] + 3, j6_vcc[1])
    add_via(board, via_3v3_ftdi, nets["3V3"])
    route_wire(board, [via_3v3_ftdi, j6_vcc], TW_POWER, F, nets["3V3"])
    route_wire(board, [via_3v3_boot, (via_3v3_ftdi[0], via_3v3_boot[1]),
                        via_3v3_ftdi], TW_POWER, B, nets["3V3"])

    # === GROUND VIAS ===
    # All ground connections go to bottom ground pour via vias
    gnd_via_parts = [
        ("C1", 2), ("C1B", 2), ("C2", 2),
        ("C3", 2), ("C3B", 2), ("C4", 2),
        ("C5", 2), ("C6", 2), ("C7", 2), ("C8", 2),
    ...
    route_wire(board, [u2_col, r4_sig], TW_SIGNAL, F, nets["WIEG_D0"])

    # U2.4 -> ESP32 GPIO25 via bottom layer
    via_d0 = (u2_col[0] - 4, u2_col[1])
    ...
    route_wire(board, [via_d0, (via_d0[0], via_d0_esp[1]), via_d0_esp],
    ...
    route_wire(board, [j2_d1, (j2_d1[0], r3_1[1]), r3_1],
    ...
    route_wire(board, [u3_col, r5_sig], TW_SIGNAL, F, nets["WIEG_D1"])

    # U3.4 -> ESP32 GPIO33 via bottom layer
    via_d1 = (u3_col[0] - 4, u3_col[1])
    ...
    route_wire(board, [via_d1, (via_d1[0], via_d1_esp[1]), via_d1_esp],
    ...
    # ESP32 -> R6 via bottom layer
    via_relay = (esp_relay[0] + 3, esp_relay[1])
    add_via(board, via_relay, nets["RELAY_DRV"])
    route_wire(board, [esp_relay, via_relay], TW_SIGNAL, F, nets["RELAY_DRV"])
    via_relay2 = (r6_1[0], r6_1[1] + 3)
    add_via(board, via_relay2, nets["RELAY_DRV"])
    route_wire(board, [via_relay, (via_relay[0], via_relay2[1]), via_relay2],
    ...
    route_wire(board, [q1_col, d2_a], TW_POWER, F, nets["RELAY_12V"])
    route_wire(board, [d2_a, (j3_1[0], d2_a[1]), j3_1], TW_POWER, F, nets["RELAY_12V"])

    # D2.1 (Cathode) -> 12V (already connected via bottom layer above)
    # J3.2 -> 12V (relay common)
    j3_2 = get_pad_pos(parts["J3"], 2)
    route_wire(board, [d2_k, (d2_k[0], j3_2[1]), j3_2], TW_POWER, F, nets["12V"])

    # === RS485 PATH ===
    # ESP32 GPIO17 (pin 28 = TX) -> U5.4 (DI)
    ...
    route_wire(board, [via_tx485, (via_tx485[0], via_tx485_2[1]), via_tx485_2],
    ...
    route_wire(board, [esp_rx485, (esp_rx485[0] + 3, esp_rx485[1]), via_rx485],
               TW_SIGNAL, F, nets["RS485_RX"])
    via_rx485_2 = (u5_ro[0] - 3, u5_ro[1])
    add_via(board, via_rx485_2, nets["RS485_RX"])
    route_wire(board, [via_rx485, (via_rx485[0], via_rx485_2[1]), via_rx485_2],
    ...
    route_wire(board, [u5_a, r7_a], TW_SIGNAL, F, nets["RS485_A"])
    route_wire(board, [r7_a, (j5_a[0], r7_a[1]), j5_a], TW_SIGNAL, F, nets["RS485_A"])

    # U5.7 (B) -> R7.2 and -> J5.2
    u5_b = get_pad_pos(parts["U5"], 7)
    r7_b = get_pad_pos(parts["R7"], 2)
    j5_b = get_pad_pos(parts["J5"], 2)
    route_wire(board, [u5_b, r7_b], TW_SIGNAL, F, nets["RS485_B"])
    route_wire(board, [r7_b, (j5_b[0], r7_b[1]), j5_b], TW_SIGNAL, F, nets["RS485_B"])

    # === FTDI / AUTO-RESET PATH ===
    # J6.4 (TXO) -> ESP32 RXD0 (pin 34)
    j6_tx = get_pad_pos(parts["J6"], 4)
    esp_rxd0 = get_pad_pos(parts["U4"], 34)
    via_ftdi_tx = (j6_tx[0] + 3, j6_tx[1])
    ...
    route_wire(board, [via_ftdi_tx, (via_ftdi_tx[0], via_ftdi_tx2[1]), via_ftdi_tx2],
    ...
    route_wire(board, [via_ftdi_rx, (via_ftdi_rx[0], via_ftdi_rx2[1]), via_ftdi_rx2],
    ...
    route_wire(board, [c9_2, r9_1], TW_SIGNAL, F, nets["ESP_GPIO0"])
    ...
    route_wire(board, [via_gpio0, (via_gpio0[0], via_gpio0_esp[1]), via_gpio0_esp],
    ...
    route_wire(board, [c10_2, r8_1], TW_SIGNAL, F, nets["ESP_EN"])
    ...
    route_wire(board, [via_en, (via_en[0], via_en_esp[1]), via_en_esp],
               TW_SIGNAL, B, nets["ESP_EN"])
...
def add_board_outline(board):
    ...
    for i in range(4):
        ...
        shape.SetShape(pcbnew.SHAPE_T_ARC if False else pcbnew.SHAPE_T_RECT)
...
def add_antenna_keepout(board):
    ...
    Module at (25, 28), body 18x25.5mm, antenna in top ~5.5mm.
    """
    zone = pcbnew.ZONE(board)
    ...
    # Antenna area: module center at (25, 28), top edge at y=28-12.75=15.25
    # Antenna extends from y=15.25 to about y=15.25+5.5=20.75
    # Add 2mm margin on sides and top
    ant_left = 25.0 - 11.0   # module left - 2mm margin
    ant_right = 25.0 + 11.0  # module right + 2mm margin
    ant_top = 15.25 - 2.0    # antenna top - 2mm margin
    ant_bottom = 21.0         # bottom of antenna area
...
def add_silkscreen_labels(board, parts):
    ...
    # Board title
    add_text(35.0, 1.5, "Conway Access Controller v2", size=1.2, bold=True)

    # J1: +12V power
    j1 = get_pad_pos(parts["J1"], 1)
    add_text(j1[0] - 2.5, j1[1] - 5.5, "+12V", size=0.8)
    add_text(j1[0] - 5.0, j1[1] + 5.5, "+", size=0.7)
    add_text(j1[0], j1[1] + 5.5, "-", size=0.7)

    # J2: Wiegand
    j2c = get_pad_pos(parts["J2"], 2)  # center-ish
    add_text(j2c[0], j2c[1] - 6.0, "WIEGAND", size=0.7)
    add_text(j2c[0] - 7.5, j2c[1] + 6.0, "D0", size=0.6)
    add_text(j2c[0] - 2.5, j2c[1] + 6.0, "D1", size=0.6)
    add_text(j2c[0] + 2.5, j2c[1] + 6.0, "+", size=0.6)
    add_text(j2c[0] + 7.5, j2c[1] + 6.0, "-", size=0.6)

    # J3: Relay
    j3c = get_pad_pos(parts["J3"], 1)
    add_text(j3c[0], j3c[1] - 5.5, "RELAY", size=0.8)
    add_text(j3c[0] - 2.5, j3c[1] + 5.5, "NO", size=0.6)
    add_text(j3c[0] + 2.5, j3c[1] + 5.5, "C", size=0.6)

    # J5: RS485
    j5c = get_pad_pos(parts["J5"], 2)
    add_text(j5c[0] - 2.0, j5c[1] - 6.0, "RS485", size=0.8)
    add_text(j5c[0] - 9.5, j5c[1] + 6.0, "A", size=0.6)
    add_text(j5c[0] - 4.5, j5c[1] + 6.0, "B", size=0.6)
    add_text(j5c[0] + 0.5, j5c[1] + 6.0, "G", size=0.6)
    add_text(j5c[0] + 5.5, j5c[1] + 6.0, "V", size=0.6)
...
def generate_jlcpcb_bom(parts, filename):
    ...
    lcsc_map = {
        "390R": "C17630", "1K": "C17513", "10K": "C17414", "120R": "C17437",
        "100nF": "C49678", "10uF": "C15850", "22uF": "C45783",
        "SS34": "C8678", "M7": "C95872", "AMS1117-3.3": "C6186",
        "EL817": "C63268", "SS8050": "C2150", "MAX3485": "C20682",
        "ESP32-WROOM-32E": "C529584",
    }
    ...
    fp_map = {
        "R": "0805", "C": "0805",
        "D1": "SMA", "D2": "SMA",
        "U1": "SOT-223-3", "U2": "SOP-4", "U3": "SOP-4",
        "U4": "ESP32-WROOM-32E", "U5": "SOIC-8",
        "Q1": "SOT-23",
    }
    ...
    for ref, part in sorted(parts.items()):
        ...
        fp_desc = fp_map.get(ref, fp_map.get(ref[0], ""))

        key = lcsc_pn
        if key not in bom:
            bom[key] = {"Comment": val, "Designator": [], "Footprint": fp_desc, "LCSC": lcsc_pn}
...
def main():
    ...
    print("  - J1: 12V power screw terminal (2-pin, 5mm pitch)")
    print("  - J2: Wiegand reader screw terminal (4-pin, 5mm pitch)")
    print("  - J3: Relay output screw terminal (2-pin, 5mm pitch)")
