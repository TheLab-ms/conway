...
func TestElections_AdminSettingsWorkflowAndMemberVoting(t *testing.T) {
	...
	require.NoError(t, adminPage.Locator("#question").Fill("Who should represent members?"))
	require.NoError(t, adminPage.Locator("#description").Fill("Choose the candidate you trust most."))
	optionInputs := adminPage.Locator("input[name='option_label']")
	...
	expect(t).Locator(memberPage.Locator("button:has-text('Voting closed')")).ToBeVisible()
}
...
func seedOpenElection(t *testing.T, env *TestEnv, adminID int64) string {
	...
	require.NoError(t, err)
	_, err = env.db.Exec(`INSERT INTO election_options (election_id, position, label) VALUES (?, 1, 'One'), (?, 2, 'Two')`, id, id)
	require.NoError(t, err)
	return id
}
